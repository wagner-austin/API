# data-bank-api â€” Design Document (Revised)
...
- Implementation detail: metadata is written as a bestâ€‘effort sidecar file alongside the blob (e.g., `/data/files/ab/cd/<file_id>.meta`). HEAD/INFO read from sidecar when available and fall back to recomputing `sha256` from the blob if missing/invalid.
...
- `POST /files` (multipart/form-data)
  ...
  - The server ignores the multipart filename for identity. The file is streamed to a temp file, `sha256` is computed while streaming, then the blob is fsync+renamed atomically to the final path derived from the `file_id` (sha256). Metadata is captured (bestâ€‘effort sidecar).
...
- `DELETE /files/{file_id}`
  - Deletes both the blob file (`.bin`) and metadata sidecar (`.meta`) if present.
...
### Integration
- Upload â†’ HEAD â†’ download round-trip; large-file streaming via temp-backed generators.
...
### Makefile (suggested)
- `make check`: ruff (fix) â†’ ruff format â†’ mypy strict â†’ guards â†’ tests with branch coverage.
...
### turkic-api â†’ data-bank-api (Upload)
...
**User Flow:**
1. `POST /api/v1/jobs` â†’ `{"job_id": "abc123"}`
...
### model-trainer â† data-bank-api (Download)

**Purpose:** Download corpus files from data-bank-api before training.

**Files to Modify:**

1. **`src/model_trainer/core/config/settings.py`** - Add data-bank-api config:
   ```python
...
2. **`src/model_trainer/api/schemas/runs.py`** - Add `corpus_file_id` field:
   ```python
...
3. **`src/model_trainer/core/services/data/corpus_fetcher.py`** - NEW service:
   ```python
...
4. **`src/model_trainer/orchestrators/training_orchestrator.py`** - Resolve corpus before enqueue:
   ```python
...
- Always stream (1â€“8 MB chunks); never buffer entire file.
