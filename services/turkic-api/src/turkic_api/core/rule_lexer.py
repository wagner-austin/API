...
Three details here are ICU's, not conveniences:

* ``'`` opens a quoted literal that runs to the next ``'``, so a ``;`` or ``>``
  inside one is an ordinary character. ``ar_lat.rules`` depends on this, and is
  broken by it — see ``tests/test_rule_engine_goldens.py``.
* ``''`` is one apostrophe.
...
class Token(NamedTuple):
    ...
    Attributes:
        ...
        members (frozenset[str]): The characters a ``set`` accepts, empty
            otherwise.
    """
...
def _read_set(text: str, start: int) -> tuple[frozenset[str], int]:
    """Read a ``[...]`` character set beginning at ``start``.

    Whitespace inside the brackets separates members and is discarded.
    Quoting does not apply inside a set; ``\\x`` still escapes ``x``.
    ...
    Returns:
        The members and the offset just past the closing bracket.

    Raises:
        RuleParseError: If the set is unterminated, nested, or empty.
    """
    members: set[str] = set()
    i = start + 1
    while i < len(text) and text[i] != "]":
        ch = text[i]
        if ch == "[":
            raise fail(text, i, "nested character set")
        if ch == "\\":
            if i + 1 >= len(text):
                raise fail(text, i, "trailing backslash in character set")
            members.add(text[i + 1])
            i += 2
            continue
        if not ch.isspace():
            members.add(ch)
...
def _lex_one(text: str, start: int, tokens: list[Token]) -> int:
    ...
    if ch == "\\":
        ...
        return start + 2
    if ch == "[":
        members, nxt = _read_set(text, start)
        tokens.append(Token("set", start, "", members))
        return nxt
    if ch == "$":
