...
QA_PLANS: Final[dict[str, QaPlan]] = {
    ...
    # THE INSTRUMENT THIS AXIS HAS BEEN SAID TO LACK, AND THE CORPUS WAS
    ...
    # wiki, which yields 237 -- measured 2026-09-09, floor 0.0211 against a
    # declared 0.0200, so the gate refuses it by thirteen items. The reading
    # taken from that was that the axis needs an instrument it does not have.
    #
    # MEASURED INSTEAD OF ASSUMED, same day, with the cap lifted only to see
    # what the corpus could yield:
    #
    #   ~/PROJECTS/wiki   841 documents   1,582,429 gpt2 tokens
    #   items available   3,735
    #   floor at 3,735    0.00134   -- fifteen times below the declared 0.02
    #
    # So the text was always there, in the same wiki family the extraction
    # ablation already ran its 2,627-item instrument against. What limited
    # this axis was `max_items`, a number in this file that nobody revisited
    # while four rungs, a retraction and a conclusion of "instrument-limited"
    # were built on top of it.
    #
    # `max_items` IS 4000 SO IT CANNOT BIND ON THIS CORPUS, which is the rule
    # the powered plans above state and could not follow: a cap below what the
    # corpus offers makes the CAP the measurement's limit and hides the corpus
    # behind it. At 4000 the corpus decides, and the number in this file stops
    # being an answer to a question nobody asked it.
    #
    # gpt2 RATHER THAN A LARGER RUNG, DELIBERATELY. gpt2 is where the
    # cartridge was reported to lose to every retriever, and where the
    # retraction landed. Re-running exactly that comparison on an instrument
    # that can resolve fifteen times finer is worth more than a new rung on an
    # instrument nobody trusts yet.
    "gpt2-full-wiki-qa": {
        "model_id": "gpt2",
        "window": 256,
        "held_out_stride": 4,
