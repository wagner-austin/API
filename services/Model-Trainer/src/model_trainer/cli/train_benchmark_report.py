"""Read training-step cost records and say what the attention pin costs to train.
...
WHAT THIS REPORT ADDS over the forward one: nothing structural, and that is
the point. A training step is the same four numbers under a different
experiment, so it is read by the same code, and the two tables can be set
beside each other without wondering whether a ratio was decided differently
in one of them.
"""
...
from model_trainer.cli.train_benchmark import train_prefix
...
def main(argv: Sequence[str] | None = None) -> int:
    """Print the training-step cost table for a directory of records.
