...
import model_trainer.worker.worker_entry as entry
...
def test_main_invokes_worker_with_built_config(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert any(
        log["extra"]["queue"] == TRAINER_QUEUE
        and log["extra"]["events_channel"] == default_events_channel("trainer")
    ...
    class _LoggingModule(types.ModuleType):
        ...
        def setup_logging(
            ...
            *,
            ...
            assert service_name == "model-trainer-worker"
...
def test_main_guard_executes_when_run_as_module(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    sys.modules.pop("model_trainer.worker.worker_entry", None)

    # Execute module under __main__ to hit guard
    runpy.run_module("model_trainer.worker.worker_entry", run_name="__main__")
