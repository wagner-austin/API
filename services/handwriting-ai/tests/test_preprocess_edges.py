...
def test_blank_image_raises_preprocessing_error() -> None:
    ...
    err = ei.value
    assert isinstance(err, AppError) and err.code is HandwritingErrorCode.preprocessing_failed


def test_invert_override_affects_output() -> None:
    img = _mk_square_digit()
    opts1: PreprocessOptions = {
        "invert": False,
        "center": True,
        "visualize": False,
        "visualize_max_kb": 16,
...
def test_visualize_size_cap_truncates_large_when_max_zero() -> None:
    """When max_kb is zero, visualization should always be suppressed."""
    img = _mk_square_digit()
    out = _visualize_png(img, 0)
    assert out is None
