...
def test_subprocess_runner_child_logging_works(
    ...
    """Verify child process can emit logs (regression test for setup_logging() bug).

    Without setup_logging() in _child_entry(), child processes would timeout silently
    ...
    # If setup_logging() wasn't called, child would timeout after 20s
