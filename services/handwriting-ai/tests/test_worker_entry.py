...
def test_build_config_reads_env() -> None:
    ...
    config_test_hooks.get_env = make_fake_env({"REDIS_URL": "redis://test-host:6379/0"})
...
def test_build_config_requires_redis_url() -> None:
    ...
    config_test_hooks.get_env = make_fake_env({})  # No REDIS_URL
...
def test_main_builds_config_from_env_when_not_provided() -> None:
    ...
    config_test_hooks.get_env = make_fake_env({"REDIS_URL": "redis://from-env:6379/0"})
...
def test_main_uses_test_runner_when_set() -> None:
    ...
    config_test_hooks.get_env = make_fake_env({"REDIS_URL": "redis://test-runner:6379/0"})
...
def test_main_guard_executes_main() -> None:
    ...
    config_test_hooks.get_env = make_fake_env({"REDIS_URL": "redis://runpy-guard-test:6379/0"})
