...
def _largest_component_crop(bw: Image.Image) -> Image.Image:
    ...
    if len(buf) != width * height:
        code = HandwritingErrorCode.preprocessing_failed
        raise AppError(code, "unexpected buffer size", handwriting_status_for(code))
    ...
    if best_bbox is None:
        code = HandwritingErrorCode.preprocessing_failed
        raise AppError(code, "no connected component found", handwriting_status_for(code))
