...
def _decode_obj_dict(x: JSONValue) -> dict[str, JSONValue] | None:
    if not isinstance(x, dict):
        return None
    # JSON objects always have string keys; return as-is
    return dict(x)


def _decode_int(d: dict[str, JSONValue], key: str, default: int) -> int:
    v = d.get(key)
...
def _decode_float(d: dict[str, JSONValue], key: str, default: float) -> float:
    v = d.get(key)
    if isinstance(v, bool):
        return default
    if isinstance(v, int | float):
...
def _read_cache(path: Path) -> tuple[CalibrationSignature, CalibrationResult, float] | None:
    ...
    sig: CalibrationSignature = {
        "cpu_cores": _decode_int(sig_raw, "cpu_cores", 0),
    ...
    interop_val = _decode_int(res_raw, "interop_threads", 0) if inter_raw is not None else None
    res: CalibrationResult = {
        "intra_threads": _decode_int(res_raw, "intra_threads", 1),
        "interop_threads": interop_val,
        "num_workers": _decode_int(res_raw, "num_workers", 0),
        "batch_size": _decode_int(res_raw, "batch_size", 1),
        "samples_per_sec": _decode_float(res_raw, "samples_per_sec", 0.0),
        "p95_ms": _decode_float(res_raw, "p95_ms", 0.0),
    }
...
def _write_cache(path: Path, sig: CalibrationSignature, res: CalibrationResult) -> None:
    ...
    interop_val = res["interop_threads"]
    interop_int = int(interop_val) if interop_val is not None else None
    body_res: dict[str, JSONValue] = {
        "intra_threads": int(res["intra_threads"]),
        "interop_threads": interop_int,
    ...
    path.write_text(dump_json_str(body), encoding="utf-8")
