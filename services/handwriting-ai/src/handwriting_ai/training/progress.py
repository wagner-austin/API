...
def emit_progress(epoch: int, total_epochs: int, val_acc: float | None) -> None:
    ...
    try:
        ...
        get_logger("handwriting_ai").error("progress_emitter_failed error=%s", exc)
        raise
...
def emit_batch(metrics: BatchMetrics) -> None:
    ...
    if cad > 0 and not (
        metrics["batch"] == 1
        or metrics["batch"] == metrics["total_batches"]
        or (metrics["batch"] % cad == 0)
    ):
        return
    try:
        em.emit_batch(metrics)
    except (RuntimeError, ValueError, TypeError) as exc:
        get_logger("handwriting_ai").error("progress_batch_emitter_failed error=%s", exc)
        raise
...
def emit_best(*, epoch: int, val_acc: float) -> None:
    ...
    try:
        ...
        get_logger("handwriting_ai").error("progress_best_emitter_failed error=%s", exc)
        raise
...
def emit_epoch(
    *, epoch: int, total_epochs: int, train_loss: float, val_acc: float, time_s: float
    ...
    try:
        ...
        get_logger("handwriting_ai").error("progress_epoch_emitter_failed error=%s", exc)
        raise
