...
from .train_config import TrainConfig, TrainingResult, TrainingResultMetadata
from .train_utils import _apply_affine, _build_model, _configure_threads, _ensure_image, _set_seed


class _TypedStateModule(Protocol):
    """Protocol for a module with typed state_dict."""

    def state_dict(self) -> dict[str, Tensor]: ...


def _typed_state_dict(model: Module) -> dict[str, Tensor]:
    """Get model state dict with proper tensor typing.

    Uses protocol coercion to avoid Any leakage from PyTorch's untyped stubs.
    """
    typed_model: _TypedStateModule = model
    return typed_model.state_dict()
...
def _run_training_loop(
    ...
    for ep in range(1, cfg["epochs"] + 1):
        ...
        cadence = int(cfg["progress_every_epochs"])
        ...
        if val_acc > best_val + float(cfg["min_delta"]):
            ...
            typed_sd = _typed_state_dict(model)
            best_sd = {k: v.detach().cpu().clone() for k, v in typed_sd.items()}
            epochs_no_improve = 0
...
def train_with_config(cfg: TrainConfig, bases: tuple[MNISTLike, MNISTLike]) -> TrainingResult:
    ...
    def run_forever() -> None:
        try:
            ...
            get_logger("handwriting_ai").error("set_num_interop_threads_failed msg=%s", exc)
    ...
    try:
        ...
        if interop is not None:
            log.info(
                f"threads_configured requested={cfg['threads']} intra={intra} interop={interop}"
            )
        else:
        ...
        with limit_thread_pools(limits=ec["intra_threads"]):
            ...
            try:
                ...
                best_sd = _typed_state_dict(model)
                best_val = _evaluate(model, test_loader, device)

        log.info("threadpoolctl_applied")

        # Prepare training result for downstream artifact creation
        sd = best_sd if best_sd is not None else _typed_state_dict(model)
...
def _configure_memory_guard_from_limits(cfg: TrainConfig, limits: ResourceLimits) -> None:
    ...
    # Compute from available memory; monitoring backend adapts to cgroup or
    # system metrics, so configuration here is independent of container mode.
