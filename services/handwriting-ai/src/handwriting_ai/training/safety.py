...
class MemoryGuardConfig(TypedDict):
    enabled: bool
...
def compute_memory_guard_config(memory_bytes: int | None) -> MemoryGuardConfig:
    ...
    if memory_bytes is None:
        # No cgroup limit detected - use conservative default suitable for
        # host environments while relying on process-scoped pressure checks.
        return {"enabled": True, "threshold_percent": 88.0, "required_consecutive": 3}
    ...
    return {"enabled": True, "threshold_percent": threshold, "required_consecutive": 3}


_cfg: MemoryGuardConfig = {
    "enabled": False,
