...
def test_build_config_reads_env() -> None:
    ...
    assert cfg["queue_name"] == QR_QUEUE
    assert cfg["events_channel"] == default_events_channel(JobDomain.QR)
...
def test_run_worker_logs_and_calls_runner() -> None:
    ...
    assert extra["queue"] == QR_QUEUE
    assert extra["events_channel"] == default_events_channel(JobDomain.QR)
...
def test_main_guard_executes_main() -> None:
    ...
    module_name = "qr_api.worker_entry"
    saved_module = sys.modules.pop(module_name, None)

    # Run the module as __main__ - this executes line 97-98
    runpy.run_module(
        module_name,
        run_name="__main__",
        alter_sys=False,
