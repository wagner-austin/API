...
from transcript_api.worker_entry import (
    _build_config,
    _run_worker,
    main,
)
...
def test_build_config_reads_env() -> None:
    ...
    assert cfg["events_channel"] == default_events_channel(JobDomain.TRANSCRIPT)
...
def test_run_worker_logs_and_calls_runner() -> None:
    ...
    assert extra["queue"] == TRANSCRIPT_QUEUE
    assert extra["events_channel"] == default_events_channel(JobDomain.TRANSCRIPT)
...
def test_main_builds_config_from_env_when_not_provided() -> None:
    ...
    assert runner.configs[0]["queue_name"] == TRANSCRIPT_QUEUE
...
def test_main_guard_executes_main() -> None:
    ...
    module_name = "transcript_api.worker_entry"
