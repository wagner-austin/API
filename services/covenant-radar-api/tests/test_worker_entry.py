...
def test_build_config_reads_env() -> None:
    ...
    assert cfg["queue_name"] == COVENANT_QUEUE
    assert cfg["events_channel"] == default_events_channel(JobDomain.COVENANT)
...
def test_run_worker_logs_and_calls_runner() -> None:
    ...
    config: WorkerConfig = {
        ...
        "queue_name": COVENANT_QUEUE,
        "events_channel": default_events_channel(JobDomain.COVENANT),
    }
    ...
    assert extra["queue"] == COVENANT_QUEUE
    assert extra["events_channel"] == default_events_channel(JobDomain.COVENANT)
...
def test_main_builds_config_from_env_when_not_provided() -> None:
    ...
    assert runner.configs[0]["queue_name"] == COVENANT_QUEUE
