...
[^6]: **Partly recoverable — corrected 2026-08-05.** The surviving artifact is `libs/cleargbm/docs/BENCHMARK_MANIFEST_2026-07-21_phase_e.json` § `all_runs`: six runs (cleargbm and lightgbm × seeds), `fit_time_s` median 1.784s vs 0.766s, with `config`, `csv_sha256`, `seeds` and `cleargbm_compute_path` ("rust (column-major + uint8 + unrolled hot-loop)") all pinned. This footnote previously said the phase-boundary manifests "were later overwritten" — **that is wrong**; the Phase E manifest exists and is pinned in this page's own `source_git_blobs`. What genuinely is NOT recoverable is the per-experiment fit-time deltas (the −4% / −5% / −16% / +23% / +15% figures), which were taken in-session between phase boundaries and never written anywhere[^1][^2]. They are recorded here for the design decisions they drove and the direction they indicate, not as reproducible measurements. The same failure mode is documented for a different figure in [[cleargbm-f32-score-narrowing-reverted]] § "Why it was reverted", and the harness that would make such numbers reproducible is [[cleargbm-leaf-normalized-benchmarking]].
[^7]: `~/PROJECTS/tech-wiki/pages/lightgbm-construct-histogram-inner.md` — the tech-wiki page both hypotheses in this section are drawn from (named inline at the "Hypothesis" paragraph above). It sits outside this workspace root, so it cannot appear in `source_paths` and this wiki's audit cannot resolve it; the `tech` wiki audits it under its own `html-sha256` contract. Cited as provenance for where the hypothesis came from, not as a verified claim of this page.
[^8]: `libs/covenant_ml/docs/BENCHMARK_MANIFEST_2026-07-24.json` § `results[]` — the authoritative 2026-07-24 re-measurement. The quoted figures are the across-seed mean ± sample standard deviation over seeds `[42, 43, 44]` of `results[].timing.median_s` and `results[].mean_leaves`; **recomputed from the manifest 2026-08-05 and they reproduce exactly** (cleargbm 1.2809 ± 0.0638 s / 47.15 leaves, lightgbm 0.8981 ± 0.0719 s / 30.96 leaves; raw 1.426×, per-leaf 0.937×). Narrative and the rebuilt median-of-repeats protocol at [[cleargbm-leaf-normalized-benchmarking]], including why this session's numbers are not directly comparable.
