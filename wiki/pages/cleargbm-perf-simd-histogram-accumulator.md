...
Weigh it against a real risk: ClearGBM has now measured **four losses** from splitting fused hot-loop work — ordered-arrays unpooled (−4%), counts-backfill (−5%), `cnt_factor` reconstruction (−16%), and on 2026-08-22 the count-split relayout (**−20%**: shrinking `BinAccumulator` to a 16-byte grad/hess pair with counts in a parallel L1-resident `u32` array, measured 0.7276s vs 0.6061s baseline under clean anchors — the second read-modify-write per bin update costs far more than the 24-byte record's cache-line straddle saves; see `libs/cleargbm/docs/BENCHMARK_RESULTS_2026-08-22_lever_sweep.md`) — plus the f32 narrowing revert[^6][^3]. The same 2026-08-22 sweep also measured the safe bounds-elision restructure of the fused loop (zip + `chunks_exact_mut`) at ≤1%, not separable from noise. A bin-grouping pre-pass is exactly the losing shape: one more pass over the data to make a later pass cheaper. The recorded meta-lesson is that this consistently loses more to overhead than it gains, and Approach 2 has no measurement contradicting it.
...
[^7]: `libs/covenant_ml/docs/BENCHMARK_MANIFEST_2026-07-24.json` § `results[]` — authoritative 2026-07-24 measurement: lightgbm 0.8981s ± 0.0719 / 30.96 leaves, cleargbm 1.2809s ± 0.0638 / 47.15 leaves; raw ratio 1.426×, per-leaf ratio 0.937×. Figures are the across-seed mean ± sample stdev over seeds `[42, 43, 44]` of `timing.median_s` and `mean_leaves`, recomputed from the manifest 2026-08-05 and reproducing exactly. Harness and protocol narrative at [[cleargbm-leaf-normalized-benchmarking]].
