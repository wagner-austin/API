...
**Read this first.** 4 topic hubs, 16 content pages. Follow the hub link for your topic; each hub lists its pages with one-line descriptions. This wiki documents the *api monorepo* — services, clients, shared libs, and the infrastructure that ties them together.
...
[Libs](hubs/libs.md) -- shared platform_* libraries (core, workers, ml, discord, music, email, calendar, codebase, devpost, kaggle, stt, langid, translate) + domain libs (covenant_domain/ml/nn/persistence, cleargbm, cleargbm_rs, procart) + instrument_io + monorepo_guards (14 pages)
