...
[Attaching a corpus to a model](../pages/corpus-attachment-program.md) -- the program index over four arms at one question: weight injection (no -- 0.24% of the wiki is triple-shaped, and AKEW scores prose in single digits), steering vectors (no -- nothing left at four composed vectors), cartridges (no -- 0.6389 against BM25's 0.8333 and slower, though it still halves answer-surprise at p = 0.0066), persona adapter (untested); plus the lever nobody has pulled, which is the corpus FORMAT rather than the method
