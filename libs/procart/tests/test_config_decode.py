...
def test_decode_tone_exposure_gamma_ok() -> None:
    raw: dict[str, float | int | bool | str] = {
        "type": "exposure_gamma",
        "exposure": 1.2,
        "gamma": 2.2,
    }
...
def test_decode_tone_reinhard_ok() -> None:
    raw: dict[str, float | int | bool | str] = {"type": "reinhard", "exposure": 0.8}
    cfg = decode_tone_mapping(raw)
...
def test_decode_tone_filmic_ok() -> None:
    raw: dict[str, float | int | bool | str] = {"type": "filmic", "exposure": 1.0}
    cfg = decode_tone_mapping(raw)
...
def test_decode_tone_type_wrong_type_raises() -> None:
    with pytest.raises(ValueError):
        bad: dict[str, float | int | bool | str] = {"type": 123, "exposure": 1.0}
...
def test_decode_tone_filmic_exposure_non_number_raises() -> None:
    with pytest.raises(ValueError):
        bad: dict[str, float | int | bool | str] = {"type": "filmic", "exposure": "x"}
        decode_tone_mapping(bad)


def test_decode_tone_exposure_gamma_rejects_bool_as_number() -> None:
    with pytest.raises(ValueError):
        bad: dict[str, float | int | bool | str] = {
            "type": "exposure_gamma",
            "exposure": True,
            "gamma": 2.2,
        }
