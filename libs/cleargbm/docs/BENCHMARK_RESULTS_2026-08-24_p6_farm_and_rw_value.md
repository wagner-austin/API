# P6 Landings A + B1 — the HPC3 farm and the rw_value corpus (2026-08-24)
...
- **Weather**: no local corpus exists; the radar weather domain is
  inference-side. The training corpus means a GHCN-D fetch and a
  McKinnon-style construction — a subsequent landing.
...
- covenant-radar-api: 2592 tests, 100.00% at close (the race fix was
  additionally held to three consecutive green full runs with the live
  broker up, at 2590 tests before the farm-filename tests landed).
