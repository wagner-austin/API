...
class TestComputeBinEdges:
    ...
    def test_computes_quantile_edges(self) -> None:
        ...
        x: tuple[tuple[float, ...], ...] = ((0.0,), (1.0,), (2.0,), (3.0,))
    ...
    def test_handles_single_feature(self) -> None:
        ...
        x: tuple[tuple[float, ...], ...] = ((0.0,), (0.5,), (1.0,), (1.5,))
    ...
    def test_avoids_duplicate_edges(self) -> None:
        ...
        x: tuple[tuple[float, ...], ...] = ((1.0,), (1.0,), (1.0,), (1.0,))
        edges = compute_bin_edges(x, n_features=1, max_bins=4)
    ...
    def test_all_nan_feature_returns_empty_edges(self) -> None:
        ...
        nan = math.nan
        x: tuple[tuple[float, ...], ...] = ((nan,), (nan,), (nan,), (nan,))
        edges = compute_bin_edges(x, n_features=1, max_bins=4)
...
class TestBinSamples:
    ...
    def test_assigns_samples_to_bins(self) -> None:
        ...
        x: tuple[tuple[float, ...], ...] = ((0.0,), (0.6,), (1.6,), (2.6,))
        edges = (BinEdges(edges=(0.5, 1.5, 2.5)),)
        bins = bin_samples(x, edges)

        assert len(bins) == 1
        assert bins[0] == (0, 1, 2, 3)

    def test_handles_multiple_features(self) -> None:
        """Should bin each feature independently."""
        x: tuple[tuple[float, ...], ...] = ((0.0, 2.6), (2.6, 0.0))
        edges = (
        ...
        assert len(bins) == 2
        assert bins[0] == (0, 3)  # Feature 0: 0.0 -> bin 0, 2.6 -> bin 3
        assert bins[1] == (3, 0)  # Feature 1: 2.6 -> bin 3, 0.0 -> bin 0
...
class TestPrecomputeFeatureBins:
    ...
    def test_precomputes_bins(self) -> None:
        ...
        assert len(bins.sample_bins) == 2
        assert len(bins.sample_bins[0]) == 4
        assert len(bins.sample_bins[1]) == 4
        # Verify bin assignments are valid bin IDs
        for feature_bins in bins.sample_bins:
            for bin_id in feature_bins:
                assert 0 <= bin_id <= 3

    def test_handles_empty_x(self) -> None:
        """Should handle empty feature matrix."""
        bins = precompute_feature_bins((), max_bins=4)

        assert bins.bin_edges == ()
        assert bins.sample_bins == ()
...
class TestBuildHistogram:
    ...
    def test_builds_histogram(self) -> None:
        ...
        hessians = (0.1, 0.2, 0.3, 0.4)
        sample_bins = (0, 0, 1, 1)  # Samples 0,1 in bin 0; samples 2,3 in bin 1
        ...
        assert histogram.gradient_sums == (3.0, 7.0)  # 1+2=3, 3+4=7
        assert _approx_tuple_equal(histogram.hessian_sums, (0.3, 0.7))  # 0.1+0.2, 0.3+0.4
        assert histogram.counts == (2, 2)

    def test_handles_subset_of_samples(self) -> None:
        """Should only include specified sample indices."""
        sample_indices = (0, 2)  # Only samples 0 and 2
        gradients = (1.0, 2.0, 3.0, 4.0)
        hessians = (0.1, 0.2, 0.3, 0.4)
        sample_bins = (0, 0, 1, 1)
        ...
        assert histogram.gradient_sums == (1.0, 3.0)  # Only samples 0 and 2
        assert _approx_tuple_equal(histogram.hessian_sums, (0.1, 0.3))
        assert histogram.counts == (1, 1)

    def test_handles_empty_bins(self) -> None:
        """Should handle bins with no samples."""
        sample_indices = (0, 1)  # Only in bin 0
        gradients = (1.0, 2.0, 3.0, 4.0)
        hessians = (0.1, 0.2, 0.3, 0.4)
        sample_bins = (0, 0, 1, 1)
        ...
        assert histogram.gradient_sums == (3.0, 0.0)
        assert _approx_tuple_equal(histogram.hessian_sums, (0.3, 0.0))
...
class TestSubtractHistogram:
    ...
    def test_subtracts_histograms(self) -> None:
        ...
        assert sibling.gradient_sums == (3.0, 6.0, 9.0)
        assert _approx_tuple_equal(sibling.hessian_sums, (0.3, 0.6, 0.9))
        assert sibling.counts == (3, 6, 9)
    ...
    def test_handles_zero_child(self) -> None:
        ...
        assert sibling.gradient_sums == parent.gradient_sums
        assert sibling.hessian_sums == parent.hessian_sums
        assert sibling.counts == parent.counts
...
class TestPartitionByBin:
    ...
    def test_partitions_samples(self) -> None:
        ...
        assert left == (0, 1)  # Samples in bins <= 0
        assert right == (2, 3)  # Samples in bins > 0
    ...
    def test_handles_subset_of_indices(self) -> None:
        ...
        assert left == (0,)  # Sample 0 in bin 0
        assert right == (2,)  # Sample 2 in bin 1
    ...
    def test_nan_samples_go_left_when_direction_is_left(self) -> None:
        ...
        sample_bins = (0, nan_bin, 1, nan_bin)
...
class TestHistogram:
    """Tests for Histogram namedtuple."""
    ...
    def test_create_histogram(self) -> None:
        ...
        assert hist.gradient_sums == (1.0, 2.0, 3.0)
        assert hist.hessian_sums == (0.1, 0.2, 0.3)
        assert hist.counts == (10, 20, 30)
...
class TestFeatureBins:
    ...
    def test_create_feature_bins(self) -> None:
        ...
        assert len(bins.bin_edges) == 1
        assert len(bins.sample_bins) == 1
        assert bins.sample_bins[0] == (0, 1, 0, 1)
