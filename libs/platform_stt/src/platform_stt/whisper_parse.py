"""Parsing utilities for OpenAI Whisper API responses.

Converts raw SDK responses into strictly typed VerboseResponse objects.
"""

from __future__ import annotations

from platform_core.json_utils import JSONTypeError
from platform_core.logging import get_logger

from .types import (
    RawVerboseDict,
    SupportsModelDump,
    SupportsToDictRecursive,
    TranscriptSegment,
    VerboseResponse,
    VerboseSegment,
)

# Extended raw dict type that includes nested dicts with various value types (from SDK)
# Segment dicts may contain unexpected types that require runtime validation
RawVerboseExtended = dict[
    str,
    str
    | int
    | float
    | bool
    | None
    | list[dict[str, str | int | float | dict[str, int] | list[int]] | int],
]
RawVerboseInput = RawVerboseDict | RawVerboseExtended


def _as_float(val: int | float | str | None) -> float:
    """Convert value to float, handling various input types.

    Args:
        val: Value to convert (int, float, numeric string, or None).

    Returns:
        Float value, or 0.0 if conversion fails.
    """
    if isinstance(val, int | float):
        return float(val)
    if isinstance(val, str):
        s = val.strip()
        if s and _is_numeric_str(s):
            return float(s)
    return 0.0


def _is_numeric_str(s: str) -> bool:
    """Check if string represents a valid numeric value.

    Args:
        s: String to check.

    Returns:
        True if string is a valid number, False otherwise.
    """
    digits = set("0123456789")
    if not s:
        return False
    if s[0] in "+-":
        s = s[1:]
        if not s:
            return False
    dot_seen = False
    digit_seen = True  # A numeric string must have at least one digit
    for ch in s:
        if ch == ".":
            if dot_seen:
                return False
            dot_seen = True
        elif ch in digits:
            digit_seen = True
        else:
            return False
    return digit_seen


def _coerce_verbose_response(raw: RawVerboseInput) -> VerboseResponse:
    """Validate and coerce a raw dict into a VerboseResponse.

    Performs strict runtime validation - raises JSONTypeError on any invalid structure.

    Args:
        raw: Raw dictionary from OpenAI SDK response.

    Returns:
        Validated VerboseResponse.

    Raises:
        JSONTypeError: If response structure is invalid.
    """
    text_val = raw.get("text")
    if not isinstance(text_val, str):
        raise JSONTypeError("verbose response missing text")
    segs_val = raw.get("segments")
    if not isinstance(segs_val, list):
        raise JSONTypeError("verbose response missing segments list")
    segs: list[VerboseSegment] = []
    for it in segs_val:
        if not isinstance(it, dict):
            raise JSONTypeError("segment must be object")
        text = str(it.get("text", ""))
        start_raw = it.get("start")
        end_raw = it.get("end")
        if start_raw is None or end_raw is None:
            raise JSONTypeError("segment missing start/end")
        if not isinstance(start_raw, (int, float, str)):
            raise JSONTypeError("segment start must be numeric")
        if not isinstance(end_raw, (int, float, str)):
            raise JSONTypeError("segment end must be numeric")
        start = _as_float(start_raw)
        end = _as_float(end_raw)
        segs.append(VerboseSegment(text=text, start=start, end=end))
    return VerboseResponse(text=text_val, segments=segs)


def to_verbose_response(
    obj: RawVerboseDict | SupportsToDictRecursive | SupportsModelDump | list[str],
) -> VerboseResponse:
    """Convert OpenAI SDK response into a strictly typed verbose response.

    Accepts dict-compatible payloads exposed by common SDKs via methods like
    `to_dict_recursive()` or `model_dump()`; otherwise raises JSONTypeError.
    Validates input at runtime - any invalid structure raises JSONTypeError.

    Args:
        obj: SDK response object or raw dict.

    Returns:
        Validated VerboseResponse.

    Raises:
        JSONTypeError: If object cannot be converted or is invalid.
    """
    if isinstance(obj, SupportsToDictRecursive):
        return _coerce_verbose_response(obj.to_dict_recursive())
    if isinstance(obj, SupportsModelDump):
        return _coerce_verbose_response(obj.model_dump())
    if isinstance(obj, dict):
        return _coerce_verbose_response(obj)
    raise JSONTypeError("Unsupported verbose object")


def convert_verbose_to_segments(data: VerboseResponse) -> list[TranscriptSegment]:
    """Convert VerboseResponse segments to TranscriptSegment list.

    Filters out empty text segments and calculates duration from start/end.

    Args:
        data: Validated VerboseResponse.

    Returns:
        List of TranscriptSegment with text, start, and duration.
    """
    segs_raw: list[VerboseSegment] = data["segments"]
    out: list[TranscriptSegment] = []
    for item in segs_raw:
        text = str(item["text"]).strip()
        if not text:
            continue
        start = float(item["start"])
        end = float(item["end"])
        duration = max(0.0, end - start)
        out.append(TranscriptSegment(text=text, start=start, duration=duration))
    return out


logger = get_logger(__name__)


__all__ = [
    "RawVerboseExtended",
    "RawVerboseInput",
    "convert_verbose_to_segments",
    "to_verbose_response",
]
