"""LSTM regressor objective function for hyperparameter optimization.

Parallel to lstm_objective.py (classification). Key differences:
- y_targets: NDArray[np.float64] (continuous, not int64 binary)
- Uses create_lstm_regressor_backend() instead of create_lstm_backend()
...
from covenant_nn.backends.lstm import create_lstm_regressor_backend

_log = get_logger(__name__)


class LSTMRegressorObjective:
    """LSTM regressor objective that trains models and returns negative RMSE.
    ...
    def __init__(
        ...
        if feature_preset != "none":
            ...
            _log.info(
                "Applied feature engineering for LSTM regressor",
                extra={
    ...
    def __call__(
        ...
        """Train LSTM regressor with given hyperparameters and return negative RMSE.
        ...
        # Train using LSTM regressor backend with temporary output directory
...
def create_lstm_regressor_objective(
    x_features: NDArray[np.float64],
    y_targets: NDArray[np.float64],
    feature_names: list[str],
    device: Literal["cpu", "cuda", "auto"],
