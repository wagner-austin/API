...
class TestLoadWandbModule:
    ...
    def test_raises_when_wandb_not_installed(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
def _is_fake_table(val: float | int | str | bool | _FakeTable) -> TypeIs[_FakeTable]:
    """Type narrowing function to identify _FakeTable values.

    TypeIs narrows both if and else branches, unlike TypeGuard.
    """
...
class _FakeWandbModule:
    ...
    def __init__(self, *, run_active: bool = True) -> None:
        ...
        self._logged_tables: list[_FakeTable] = []
    ...
    def log(self, data: Mapping[str, float | int | str | bool | _FakeTable]) -> None:
        # Separate tables from scalar metrics using TypeGuard
        scalar_data: dict[str, float | int | str | bool] = {}
        for key, val in data.items():
            if _is_fake_table(val):
                # TypeGuard narrows val to _FakeTable
                self._logged_tables.append(val)
                # Store table reference in scalar logs for verification
                scalar_data[key] = f"<table:{len(self._logged_tables) - 1}>"
            else:
                # TypeGuard narrows val to float | int | str | bool
                scalar_data[key] = val
    ...
    def __getattr__(self, name: str) -> type[_FakeTable]:
        """Handle attribute access for 'Table' to match wandb API."""
        if name == "Table":
...
def _setup_fake_wandb(monkeypatch: pytest.MonkeyPatch, fake_wandb: _FakeWandbModule) -> None:
    """Set up fake wandb module in sys.modules."""
    import importlib.util
    import sys
...
def test_enabled_publisher_init(monkeypatch: pytest.MonkeyPatch) -> None:
    """Enabled publisher should initialize wandb run."""
    fake_wandb = _FakeWandbModule(run_active=True)
    _setup_fake_wandb(monkeypatch, fake_wandb)
...
def test_enabled_publisher_get_init_result(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_log_config(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_log_step(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_log_epoch(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_log_final_true(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_log_final_false(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_log_table(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_enabled_publisher_finish(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_log_config_noop_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
    """log_config should be no-op when wandb.run is None."""
    fake_wandb = _FakeWandbModule(run_active=False)
    _setup_fake_wandb(monkeypatch, fake_wandb)
...
def test_log_step_noop_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_log_epoch_noop_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_log_final_noop_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_log_table_noop_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_finish_noop_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_is_enabled_false_when_run_none(monkeypatch: pytest.MonkeyPatch) -> None:
