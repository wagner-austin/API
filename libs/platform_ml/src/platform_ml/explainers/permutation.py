"""Permutation importance explainer.

Computes feature importance by measuring prediction change when shuffling features.
Model-agnostic: works with any model implementing PredictorProtocol.

Algorithm:
1. Compute baseline predictions on original data
2. For each feature:
   a. Shuffle the feature column n_repeats times
   b. Compute predictions on shuffled data
   c. Measure mean absolute change in target class probability
3. Rank features by mean importance across repeats

Higher importance means the model relies more heavily on that feature.
"""

from __future__ import annotations

import numpy as np
from numpy.typing import NDArray

from .protocol import PredictorProtocol
from .types import (
    ExplainerCapabilities,
    ExplainerName,
    FeatureImportanceScore,
    PermutationConfig,
)

PERMUTATION_CAPABILITIES: ExplainerCapabilities = {
    "requires_gradients": False,
    "requires_background_data": False,
    "computational_cost": "medium",
}


def _validate_inputs(
    x_data: NDArray[np.float64],
    feature_names: list[str],
) -> None:
    """Validate input data and feature names match.

    Args:
        x_data: Input features array.
        feature_names: List of feature names.

    Raises:
        ValueError: If dimensions don't match.
    """
    n_features = int(x_data.shape[1])
    if len(feature_names) != n_features:
        raise ValueError(
            f"feature_names length ({len(feature_names)}) must match x_data columns ({n_features})"
        )


def _compute_baseline_proba(
    model: PredictorProtocol,
    x_data: NDArray[np.float64],
    target_class: int,
) -> NDArray[np.float64]:
    """Compute baseline probabilities for target class.

    Args:
        model: Model with predict_proba method.
        x_data: Input features.
        target_class: Class index.

    Returns:
        Probabilities for target class with shape (n_samples,).
    """
    proba: NDArray[np.float64] = model.predict_proba(x_data)
    return proba[:, target_class].astype(np.float64)


def _compute_single_feature_importance(
    model: PredictorProtocol,
    x_data: NDArray[np.float64],
    baseline_proba: NDArray[np.float64],
    feature_idx: int,
    target_class: int,
    n_repeats: int,
    rng: np.random.Generator,
) -> float:
    """Compute importance for a single feature.

    Args:
        model: Model with predict_proba method.
        x_data: Input features.
        baseline_proba: Baseline probabilities for target class.
        feature_idx: Index of feature to permute.
        target_class: Class index.
        n_repeats: Number of permutation repeats.
        rng: Random number generator.

    Returns:
        Mean absolute change in probability when feature is shuffled.
    """
    n_samples = int(x_data.shape[0])
    importance_scores: list[float] = []

    for _ in range(n_repeats):
        # Create copy and shuffle the feature column
        x_permuted: NDArray[np.float64] = x_data.copy()
        # Use np.intp for permutation indices (standard numpy pattern)
        shuffled_indices: NDArray[np.intp] = rng.permutation(n_samples)
        # Assign shuffled column with proper typing
        shuffled_column: NDArray[np.float64] = x_data[shuffled_indices, feature_idx]
        x_permuted[:, feature_idx] = shuffled_column

        # Compute predictions on permuted data
        permuted_proba: NDArray[np.float64] = model.predict_proba(x_permuted)
        permuted_class_proba: NDArray[np.float64] = permuted_proba[:, target_class].astype(
            np.float64
        )

        # Compute mean absolute change using sum/len pattern
        diff: NDArray[np.float64] = np.abs(baseline_proba - permuted_class_proba)
        mean_diff: float = float(np.sum(diff)) / n_samples
        importance_scores.append(mean_diff)

    # Compute mean of importance scores using Python
    return sum(importance_scores) / len(importance_scores)


def _get_importance_from_pair(pair: tuple[str, float]) -> float:
    """Extract importance value from (name, importance) tuple.

    Args:
        pair: Tuple of (feature_name, importance_score).

    Returns:
        The importance score.
    """
    return pair[1]


def _rank_importances(
    feature_names: list[str],
    importances: list[float],
) -> list[FeatureImportanceScore]:
    """Rank features by importance and return sorted list.

    Args:
        feature_names: List of feature names.
        importances: List of importance scores.

    Returns:
        List of FeatureImportanceScore sorted by rank (most important first).
    """
    # Create list of (name, importance) tuples
    name_importance_pairs: list[tuple[str, float]] = list(
        zip(feature_names, importances, strict=True)
    )

    # Sort by importance descending using typed helper function
    sorted_pairs: list[tuple[str, float]] = sorted(
        name_importance_pairs,
        key=_get_importance_from_pair,
        reverse=True,
    )

    # Build ranked results
    results: list[FeatureImportanceScore] = []
    for rank_idx, pair in enumerate(sorted_pairs):
        name: str = pair[0]
        importance: float = pair[1]
        score: FeatureImportanceScore = {
            "name": name,
            "importance": importance,
            "rank": rank_idx + 1,
        }
        results.append(score)

    return results


class PermutationExplainer:
    """Permutation importance explainer.

    Computes feature importance by measuring how much predictions change
    when each feature is randomly shuffled. Works with any model.

    Args:
        config: Configuration with n_repeats and random_state.

    Example:
        >>> config: PermutationConfig = {"n_repeats": 10, "random_state": 42}
        >>> explainer = PermutationExplainer(config)
        >>> importance = explainer.compute_importance(
        ...     model=model,
        ...     x_data=x_test,
        ...     feature_names=["feat1", "feat2"],
        ...     target_class=1,
        ... )
    """

    def __init__(self, config: PermutationConfig) -> None:
        """Initialize permutation explainer.

        Args:
            config: Configuration with n_repeats and random_state.
        """
        self._n_repeats = int(config["n_repeats"])
        self._random_state = int(config["random_state"])

    def explainer_name(self) -> ExplainerName:
        """Return explainer name.

        Returns:
            Literal "permutation".
        """
        return "permutation"

    def capabilities(self) -> ExplainerCapabilities:
        """Return explainer capabilities.

        Returns:
            Capabilities indicating no gradients required, medium cost.
        """
        return PERMUTATION_CAPABILITIES

    def compute_importance(
        self,
        *,
        model: PredictorProtocol,
        x_data: NDArray[np.float64],
        feature_names: list[str],
        target_class: int,
    ) -> list[FeatureImportanceScore]:
        """Compute permutation feature importance.

        Args:
            model: Model implementing PredictorProtocol.
            x_data: Input data with shape (n_samples, n_features).
            feature_names: List of feature names matching x_data columns.
            target_class: Class index for which to compute importance.

        Returns:
            List of FeatureImportanceScore sorted by rank.

        Raises:
            ValueError: If feature_names length doesn't match x_data columns.
        """
        _validate_inputs(x_data, feature_names)

        rng = np.random.default_rng(self._random_state)
        n_features = int(x_data.shape[1])

        # Compute baseline predictions
        baseline_proba = _compute_baseline_proba(model, x_data, target_class)

        # Compute importance for each feature
        importances: list[float] = []
        for feature_idx in range(n_features):
            importance = _compute_single_feature_importance(
                model=model,
                x_data=x_data,
                baseline_proba=baseline_proba,
                feature_idx=feature_idx,
                target_class=target_class,
                n_repeats=self._n_repeats,
                rng=rng,
            )
            importances.append(importance)

        return _rank_importances(feature_names, importances)


def create_permutation_explainer(config: PermutationConfig) -> PermutationExplainer:
    """Factory function to create a PermutationExplainer.

    Args:
        config: Configuration with n_repeats and random_state.

    Returns:
        Configured PermutationExplainer instance.
    """
    return PermutationExplainer(config)


__all__ = [
    "PERMUTATION_CAPABILITIES",
    "PermutationExplainer",
    "create_permutation_explainer",
]
