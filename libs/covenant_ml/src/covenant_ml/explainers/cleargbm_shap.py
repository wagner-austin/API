...
Converts ClearGBM's GradientBoostingModel to a format that SHAP's
TreeExplainer can parse. This allows using SHAP's fast tree-based
explanations with ClearGBM models.
...
import numpy as np
from cleargbm.types import DecisionTree, GradientBoostingModel, TreeNode
...
class ClearGBMShapWrapper:
    ...
    Converts a ClearGBM GradientBoostingModel to SHAP format and provides
    methods for computing local explanations.

    Example:
        >>> model: GradientBoostingModel = train_gradient_boosting(...)
    ...
    def __init__(self, model: GradientBoostingModel) -> None:
        """Initialize wrapper with a ClearGBM model.
        ...
        Raises:
            ValueError: If model has no trees.
        """
        self._shap_format = convert_cleargbm_to_shap_format(model)
        self._base_prediction = model["base_prediction"]
