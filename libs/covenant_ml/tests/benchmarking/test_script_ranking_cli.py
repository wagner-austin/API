...
from scripts.benchmark_cleargbm_ranking import main
...
class TestMain:
    ...
    def test_the_record_names_this_familys_own_experiment(self, tmp_path: Path) -> None:
        ...
        assert narrow_json_to_str(record["experiment"]) == "cleargbm-vs-lightgbm-ranking-quality"
    ...
    def test_the_report_names_every_arm(
        ...
        main(_small_args(out), pin=_stand_in_pin)
        encoded = load_json_str(out.read_text(encoding="utf-8"))
        names = [line.split()[0] for line in summary_lines(RANKING, encoded)]
        assert "mean_cleargbm.mean_ndcg" in names
    ...
    def test_the_entry_point_refuses_once_numpy_is_loaded(self, tmp_path: Path) -> None:
        ...
        saved = sys.argv
        sys.argv = ["benchmark_cleargbm_ranking", *_small_args(tmp_path / "m.json")]
        try:
