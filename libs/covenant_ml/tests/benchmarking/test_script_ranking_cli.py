"""Tests for the ranking benchmark command-line entry point.
...
from scripts.benchmark_cleargbm_ranking import build_parser, main
...
class TestParser:
    ...
    def test_defaults(self) -> None:
        ...
        docs: int = parsed.docs
        truncation: int = parsed.truncation
        seeds: list[int] = parsed.seeds
        out: Path | None = parsed.out
        assert n_queries == 400
        assert docs == 20
        assert truncation == 10
...
class TestMain:
    ...
    def test_writes_a_manifest(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        expected_lines = [
            "ranking corpus: 40 queries x 8 docs, 4 features, NDCG@8, seeds [42]",
        ]
        for entry in results:
            ...
            mean_ndcg = narrow_json_to_float(quality["mean_ndcg"])
            expected_lines.append(f"  {model:>9} seed={seed} mean_ndcg={mean_ndcg:.6f}")
    ...
    def test_runs_without_an_output_path(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert len(lines) == 3
        assert lines[0] == "ranking corpus: 40 queries x 8 docs, 4 features, NDCG@8, seeds [42]"
    ...
    def test_the_entry_point_refuses_once_numpy_is_loaded(self) -> None:
        ...
        saved = sys.argv
        sys.argv = ["benchmark_cleargbm_ranking", *_small_args()]
        try:
            with pytest.raises(NativeLibrariesAlreadyLoadedError):
                runpy.run_module("scripts.benchmark_cleargbm_ranking", run_name="__main__")
        finally:
            sys.argv = saved
