...
from scripts.benchmark_cleargbm_regression import main

from covenant_ml.benchmarking.declarations import REGRESSION
...
def _small_args(out: Path, external_dir: Path) -> list[str]:
...
class TestRequiredArguments:
    ...
    def test_a_run_without_seeds_is_refused(self, tmp_path: Path) -> None:
        ...
        args = list(_small_args(tmp_path / "m.json", tmp_path))
...
class TestMain:
    ...
    def test_a_run_writes_a_manifest_and_its_record(
        ...
        assert main(_small_args(out, tmp_path), pin=_stand_in_pin) == 0
        ...
        expected = (
            "\n".join(summary_lines(REGRESSION, encoded))
    ...
    def test_the_manifest_carries_every_arm(self, tmp_path: Path) -> None:
        write_rw_value_fixture(tmp_path)
        out = tmp_path / "manifest.json"
        main(_small_args(out, tmp_path), pin=_stand_in_pin)
        decoded = narrow_json_to_dict(load_json_str(out.read_text(encoding="utf-8")))
        assert len(narrow_json_to_list(decoded["results"])) == 4
    ...
    def test_the_record_names_this_familys_own_experiment(self, tmp_path: Path) -> None:
        ...
        assert narrow_json_to_str(record["experiment"]) == "cleargbm-vs-lightgbm-regression-quality"

    def test_the_report_names_every_arm(
        self, tmp_path: Path, capsys: pytest.CaptureFixture[str]
    ) -> None:
        # Rendered from the SAME observations the record carries, so a summary
        # that drifts from the artifact is impossible.
        write_rw_value_fixture(tmp_path)
        out = tmp_path / "manifest.json"
        main(_small_args(out, tmp_path), pin=_stand_in_pin)
        encoded = load_json_str(out.read_text(encoding="utf-8"))
        names = [line.split()[0] for line in summary_lines(REGRESSION, encoded)]
        assert "mean_cleargbm.rmse" in names
    ...
    def test_the_entry_point_refuses_once_numpy_is_loaded(self, tmp_path: Path) -> None:
        ...
        saved = sys.argv
        sys.argv = ["benchmark_cleargbm_regression", *_small_args(tmp_path / "m.json", tmp_path)]
        try:
