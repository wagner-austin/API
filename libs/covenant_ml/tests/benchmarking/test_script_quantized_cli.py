"""Tests for the quantized benchmark command-line entry point.
...
from scripts.benchmark_cleargbm_quantized import build_parser, main
...
class TestParser:
    ...
    def test_defaults(self) -> None:
        """The default corpus and bin count are the documented ones."""
...
class TestMain:
    ...
    def test_writes_a_manifest(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        for entry in results:
            ...
            expected_lines.append(
                f"  {model:>9}/{histogram:<9} seed={seed} auc={auc:.6f} "
                f"log_loss={log_loss:.6f} fit={fit_seconds:.3f}s"
            )
    ...
    def test_runs_without_an_output_path(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        assert len(lines) == 5
        assert lines[0] == "quantized corpus: 800 rows x 4 features, 4 gradient bins, seeds [42]"
    ...
    def test_the_entry_point_refuses_once_numpy_is_loaded(self) -> None:
        ...
        saved = sys.argv
        sys.argv = ["benchmark_cleargbm_quantized", *_small_args()]
        try:
            with pytest.raises(NativeLibrariesAlreadyLoadedError):
                runpy.run_module("scripts.benchmark_cleargbm_quantized", run_name="__main__")
        finally:
            sys.argv = saved
