"""Tests for the quantized-histogram benchmark command-line entry point.
...
from scripts.benchmark_cleargbm_quantized import main
...
class TestMain:
    ...
    def test_the_record_names_this_familys_own_experiment(self, tmp_path: Path) -> None:
        ...
        assert narrow_json_to_str(record["experiment"]) == "cleargbm-vs-lightgbm-quantized-quality"
    ...
    def test_the_report_names_every_arm(
        ...
        main(_small_args(out), pin=_stand_in_pin)
        encoded = load_json_str(out.read_text(encoding="utf-8"))
        names = [line.split()[0] for line in summary_lines(QUANTIZED, encoded)]
        assert any(name.startswith("mean_cleargbm@") for name in names)
    ...
    def test_the_entry_point_refuses_once_numpy_is_loaded(self, tmp_path: Path) -> None:
        ...
        saved = sys.argv
        sys.argv = ["benchmark_cleargbm_quantized", *_small_args(tmp_path / "m.json")]
        try:
