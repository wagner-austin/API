"""Measure multiclass quality for ClearGBM and LightGBM side by side.

A DECLARATION AND A CONFIG, nothing else. What this benchmark IS lives in
:data:`covenant_ml.benchmarking.declarations.MULTICLASS`; running it, writing its
...
def main(argv: list[str] | None = None, pin: PinProtocol = pin_single_thread) -> int:
    """Run the multiclass benchmark and write its manifest and record.
    ...
    parsed = shared_parser(MULTICLASS).parse_args(argv)
    ...
    manifest = run_multiclass_benchmark(config, seeds, fingerprint)
    encoded = encode_multiclass_manifest(manifest)
    sys.stdout.write("\n".join(summary_lines(MULTICLASS, encoded)) + "\n")
    record_path = write_manifest_and_record(MULTICLASS, encoded, fingerprint, out_path, seeds)
