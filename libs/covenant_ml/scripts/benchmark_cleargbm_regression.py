"""Measure regression quality and fit time on an external corpus.

A DECLARATION AND A CONFIG, nothing else. What this benchmark IS lives in
:data:`covenant_ml.benchmarking.declarations.REGRESSION`; running it, writing its
...
def main(argv: list[str] | None = None, pin: PinProtocol = pin_single_thread) -> int:
    """Run the regression benchmark and write its manifest and record.
    ...
    parsed = shared_parser(REGRESSION).parse_args(argv)
    ...
    config = RegressionBenchConfig(
        ...
        early_stopping_rounds=parsed.early_stopping,
    )
    ...
    record_path = write_manifest_and_record(REGRESSION, encoded, fingerprint, out_path, seeds)
