...
The rules that REFUSE a claim the index cannot support live in
``test_research_index_claims``. The split is by role: this module is about
rendering the generated block and substituting it into the document, where the
remedy for drift is to generate; that one is about the prose around it, where
nothing can be generated and the only move is to refuse.
"""
...
class TestRenderingARow:
    ...
    def test_an_image_is_shown_by_its_digest(self) -> None:
        ...
        row = render_project_row("mi", project_config(image_sha="a" * 64))
    ...
    def test_a_cpu_project_says_cpu(self) -> None:
        ...
        assert "| cpu |" in render_project_row("cleargbm", project_config())
    ...
    def test_a_gpu_is_rendered_field_by_field(self) -> None:
        ...
        """
        row = render_project_row("mi", project_config(gpu=GpuRequest(model="A100", count=1)))
...
class TestRenderingTheBlock:
    ...
    def test_projects_are_sorted_so_two_renderings_match(self) -> None:
        ...
        projects = {"zeta": project_config(), "alpha": project_config()}
    ...
    def test_the_block_carries_its_own_markers(self) -> None:
        ...
        block = render_projects_block({"alpha": project_config()})
    ...
    def test_the_block_names_how_to_regenerate_it(self) -> None:
        ...
        assert "hpc3-research-index --write" in render_projects_block({"a": project_config()})
...
class TestSubstitutingTheBlock:
    ...
    def test_the_surrounding_prose_is_untouched(self) -> None:
        ...
        result = replace_projects_block(document, render_projects_block({"a": project_config()}))
