...
``tools/hpc-wake`` and ``tools/fleet-wake``. What is here is only what is
true of THIS bridge.
...
``assertSessionLabel``). Editing :data:`_SESSION_NAME` mints a different id,
after which every post is refused with ``TASK_IDENTITY_MISMATCH`` and there
is no way to unbind the label. The value is pinned as a literal in
``tests/test_identity.py`` -- not re-derived there -- so an edit fails a test
rather than a production cycle.
"""
...
HARNESS: Final = "ci-wake"

#: One sentence on the ledger row, for whoever reads it months from now.
PURPOSE: Final = (
    "the GitHub Actions bridge, which announces each enrolled push's workflow "
    "verdict on the standing CI task and mentions the session that pushed it."
)
