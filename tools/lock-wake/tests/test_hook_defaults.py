...
The implementations themselves are platform_core's and are tested against
the real filesystem and stdout in its suite (``test_journal_cursor.py``,
``test_report_line.py``). What is this package's own is that production
binds exactly those, and that a reset restores them after a test rebinds.
"""
