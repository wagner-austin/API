...
[A determinism verdict cannot tell "reproducible" from "reproducibly broken"](../pages/a-determinism-verdict-needs-a-correctness-oracle.md) -- a run with zero contacts scores `deterministic: true`; the ten-scene table was re-checked against that and survives, and the cheap fix is a liveness witness rather than a physics oracle
