...
**Answered, and measured at every link.** The failure follows `MJ_COLLISION_TABLE` exactly: every pair routing to `CollisionType.CONVEX` loses all its contacts under `RUN_TO_RUN`, every `PRIMITIVE` pair is untouched, six for six. Instrumenting the pipeline end to end shows broadphase, the CCD slot reservation and **the EPA distance itself** all agreeing bit for bit between modes, with only the contact count differing — 4 against 0.

The cause is the *second* slot reservation, in `write_contact`: it returns before reserving when `dist_in` says nothing was detected, and `dist_in` is the EPA result, computed by iterating `epa_vert[ccdid]` / `epa_face[ccdid]` **scratch arrays**. Warp suppresses array writes during its counting pass, so that pass sees unwritten scratch, decides there is no contact, and reserves nothing; the execution pass then computes a real −0.02 penetration and finds no slot. Full detail and citations: [[deterministic-mode-drops-contacts-in-convex-narrowphase]].

**An earlier answer here was wrong and is withdrawn.** It said the CCD atomic's counter helper was absent so `nccd` never incremented. `nccd` was then measured and is identical in both modes (1, at counter index 51). Warp's own documentation also states Pattern 2 slot allocation is *supported* for exactly that atomic's shape, and a standalone kernel reproducing it allocates correctly. The CCD atomic is innocent; the failing reservation is a different one, further down.

Six hypotheses died on the way and are listed on that page so nobody re-runs them.

**What is still open here** is no longer diagnosis. The fix is undesigned — Warp's guidance is to make the slot decision independent of suppressed writes, but the decision genuinely needs a value only EPA can produce from a workspace. Whether upstream treats Warp's codegen mode as in scope at all is also open, given the `opt.deterministic` stack in question 1. And a `GPU_TO_GPU` sweep across the six pairs is unmeasured; only mesh-on-box was checked there, where it fails identically.
