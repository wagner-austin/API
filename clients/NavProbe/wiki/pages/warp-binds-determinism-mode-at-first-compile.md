...
[^1]: `[observed]` — with the fix's staged path force-disabled in a build (`deterministic = False` in `convex_narrowphase`), an in-process flip to `RUN_TO_RUN` after one default-mode `mjw.collision()` produced identical contact counts (4) in 0.61s with no recompile; the same flip done before import produces 0 contacts on the same build. warp-lang 1.15.0.
[^2]: `[observed]` — first-draft `test_deterministic_convex_narrowphase` (in-process flip): "1 passed ... in 6.57s" with fix, "1 passed ... in 0.61s" with fix disabled. A test that cannot fail is not a test.
