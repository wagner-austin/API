...
The mode it unlocks is not, on that model. Under `RUN_TO_RUN` the same fixture generates **zero contacts** and the body falls through the geom it should rest on — erratically at `deterministic_max_records = 64`, and identically on all eight repetitions at 4096 ([[deterministic-mode-drops-contacts-in-convex-narrowphase]]). So two qualifications land on this page's headline:
