...
This file is a manual decoding-status snapshot, not an auto-generated report.
...
- length-based and structure-based identification notes
- message formats that are implemented and considered stable
- capture notes that are still useful for reverse-engineering

This file should not be treated as a precise live metric dashboard.
...
The entries below are retained as reverse-engineering notes from earlier
captures. They are not a current completeness metric for the whole codebase.

## Recently Captured (Need Decoding)
