"""The session-status contract carried across the thread boundary.

The wire mode vocabulary, the live counters, and the status snapshot
the tick loop publishes each tick. Lives beside the buses rather than
in ``service`` because the bot and the HTTP layer both speak it.

The wire vocabulary adds one value on top of :data:`AIMode`: ``"AUTO"``.
On the wire it means "restore the durable HFSM auto-arbitrator";
internally it maps to ``manual_mode = None``.
"""
