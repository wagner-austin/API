"""The typed command model and its serialization.

``QueryCommand`` and ``ActionCommand`` are the structured form of a
client command — the shape the codecs validate JSON into and the
serializers turn into wire bytes. Split out of
:mod:`tankpit_bot.protocol.commands` 2026-09-03; the vocabulary it
reads (the type numbers, the prefix) stayed there.

Deleted in the same move: a ``CommandType = Literal["query", "action"]``
alias that was defined, exported, and referenced by nothing.
"""
