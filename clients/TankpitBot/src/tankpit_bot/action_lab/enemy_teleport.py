"""The enemy-teleport probe: teleport onto an enemy and record it.

Holds :class:`EnemyTeleportProbe` and its entry points. Enemy selection
and result shaping are
:mod:`tankpit_bot.action_lab.enemy_teleport_targeting`, reached through
the module so the tests that swap those functions keep a working
injection seam.
"""
...
class EnemyTeleportProbe(ProbeBase):
    ...
    def _probe_single_enemy_attempt(
        ...
        *,
        ...
        enemy = enemy_teleport_targeting._require_fresh_enemy_threat(
            self, acquisition_started_ms, excluded_tank_ids
        )
        if enemy is None:
            return self._finish_non_teleport_attempt(
                page=page,
