...
# additionally stands up the HTTP surface (/status, /watch, and the
# session's /video/{file} HLS files), which the bare bot entry has no
# way to expose because it starts no HTTP server at all.
# TANKPIT_BOT_AUTOSTART makes it run its session rather than
