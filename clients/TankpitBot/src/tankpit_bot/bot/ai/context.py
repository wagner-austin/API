...
from tankpit_bot.physics.capacity import rank_scaled_reserve
...
class DecideCtx:
    ...
    def __init__(
        ...
        *,
        ...
        # the config values are the RANK-4 REFERENCE tuning (operator-
        # tunable as before); the floors every decision reads are that
        # tuning scaled to the tank's true capacity, so a private is
        # no longer priced by a lieutenant's reserve. Derived per tick
        # because a mid-session promotion changes capacity instantly.
        rank = self_state["rank"]
        self.fuel_low_floor: int = rank_scaled_reserve(
            ai_state["config"]["fuel_low_threshold"], rank
        )
        self.hunt_reserve_floor: int = rank_scaled_reserve(
            ai_state["config"]["hunt_min_fuel"], rank
        )
        self.engagement_budget: int = rank_scaled_reserve(
            ai_state["config"]["engagement_fuel_budget"], rank
        )
