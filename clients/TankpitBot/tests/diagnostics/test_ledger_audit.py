...
def test_repeated_failures_on_one_target_are_a_retry_loop() -> None:
    """Two failures on the same (kind, target) mean replanning is not learning."""
    findings = audit_ledger(
        [
            _record(
                "2026-07-19T00:48:26",
    ...
    assert _by_check(findings, "rejection_retry_loop") == [
        make_finding(
            ...
            target_x=200,
            target_y=128,
            failures="command_rejected,discarded_no_container",
        )
    ]


def test_executor_discards_aggregate_per_kind_and_label() -> None:
    """Discard outcomes aggregate into one warning per (kind, label)."""
    findings = audit_ledger(
        [
            _record(
                "2026-07-19T00:48:00",
    ...
    assert _by_check(findings, "executor_discards") == [
        make_finding(
            "executor_discards",
            "warning",
            "executor discarded 2 shoot command(s) as "
            "discarded_target_not_tracked -- planner and executor "
            "disagreed about the world",
            action_kind="shoot",
            outcome="discarded_target_not_tracked",
            count=2,
        )
