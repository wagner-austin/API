...
def test_console_listener_logs_websocket_messages(
    ...
    assert "[Console log]" in output and "WS Hook" in output
    assert "[Console info]" in output and "WebSocket connected" in output
    # Unrelated messages should NOT be logged
    assert "Some unrelated message" not in output
...
def test_console_listener_handles_missing_value(
    ...
    assert "[Console log]" in output and "WS Hook description" in output
...
def test_console_listener_handles_non_dict_arg_element(
    ...
    assert "WS valid dict" in output
