...
def test_encode_mouse_input() -> None:
    ...
    inp = MouseInput(x=100, y=200, button=MouseButton.LEFT)
...
def test_decode_mouse_input() -> None:
    ...
    assert result["x"] == 100
    assert result["y"] == 200
    assert result["button"] is MouseButton.RIGHT
...
def test_decode_mouse_input_middle_button() -> None:
    ...
    result = decode_mouse_input(data)
    assert result["button"] is MouseButton.MIDDLE
...
def test_decode_mouse_input_invalid_button() -> None:
    ...
    assert str(excinfo.value) == (
        "Invalid button 'invalid': must be one of 'left', 'right', 'middle'"
    )
...
def test_encode_probe_input_key() -> None:
    ...
    inp = ProbeInput(input_type=InputType.KEY, key_input=key_inp, mouse_input=None)
...
def test_encode_probe_input_mouse() -> None:
    ...
    mouse_inp = MouseInput(x=300, y=400, button=MouseButton.LEFT)
    inp = ProbeInput(input_type=InputType.MOUSE, key_input=None, mouse_input=mouse_inp)
    result = encode_probe_input(inp)
    ...
    assert mouse_result["button"] == "left"
...
def test_decode_probe_input_key() -> None:
    ...
    assert result["input_type"] is InputType.KEY
...
def test_decode_probe_input_mouse() -> None:
    ...
    result = decode_probe_input(data)
    assert result["input_type"] is InputType.MOUSE
    assert result["key_input"] is None
...
def test_decode_probe_input_invalid_type() -> None:
    ...
    assert str(excinfo.value) == "Invalid input_type 'invalid': must be one of 'key', 'mouse'"
