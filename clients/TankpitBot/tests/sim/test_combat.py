...
def test_ground_shot_is_a_single_with_full_deferred_cost() -> None:
    ...
    world = _arena()
    outcome = process_shot(world, InMemoryTerrainMap(), 9, 12, 12, _NOBODY)
...
def test_terrain_clips_the_impact_tile_and_still_bills() -> None:
    ...
    outcome = process_shot(world, terrain, 9, 15, 10, _NOBODY)
    assert outcome["weapon"] == WEAPON_SINGLE
...
def test_tank_in_the_line_of_sight_obstructs() -> None:
    ...
    outcome = process_shot(world, InMemoryTerrainMap(), 9, 15, 10, _NOBODY)
    assert outcome["weapon"] == WEAPON_SINGLE
    assert (outcome["impact_x"], outcome["impact_y"]) == (12, 10)
...
def test_same_tick_mover_draws_a_homing() -> None:
    ...
    outcome = process_shot(world, InMemoryTerrainMap(), 9, 15, 10, frozenset({11}))
    assert outcome["weapon"] == WEAPON_HOMING
    assert world["tanks"][11]["fuel"] == 455
    without = _arena()
    fallback = process_shot(without, InMemoryTerrainMap(), 9, 15, 10, frozenset({11}))
...
def test_damage_progression_and_deactivation() -> None:
    ...
    first = process_shot(world, InMemoryTerrainMap(), 9, 15, 10, _NOBODY)
    assert first["victim_deactivated"] is False
    assert world["tanks"][11]["damage_state"] == 3
    second = process_shot(world, InMemoryTerrainMap(), 9, 15, 10, _NOBODY)
...
def test_critical_tier_stays_critical() -> None:
    ...
    process_shot(world, InMemoryTerrainMap(), 9, 15, 10, _NOBODY)
...
def test_shooting_a_mine_cascades_two_packets() -> None:
    ...
    outcome = process_shot(world, InMemoryTerrainMap(), 9, 13, 12, _NOBODY)
...
def test_pure_vertical_ray_resolves() -> None:
    ...
    world = _arena()
    outcome = process_shot(world, InMemoryTerrainMap(), 9, 10, 14, _NOBODY)
...
def test_shot_at_own_tile_has_an_empty_ray() -> None:
    ...
    world = _arena()
    outcome = process_shot(world, InMemoryTerrainMap(), 9, 10, 10, _NOBODY)
