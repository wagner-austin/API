...
## [2026-08-27] correction + fix | "The room was never empty": the relay only ever tried its nearest enemy

Operator ground truth ("there are always always 27 enemies... they respawn in like 1 min") demolished the "practice room ran out of NPCs" story from tonight's farm sessions. The artifact agreed: at the moment session bot-20260827-162050 exited `no_viable_targets`, the acquisition_candidates diagnostic shows ALL 27 enemies alive and positioned, every one rejected `unaffordable`, at FULL fuel 1100 with 528 map fuel dots. The map blob is complete (37 tanks, field-wide dots — no partition; arrow-scrolling is a human-UI concern only).

**Root cause, reproduced offline against the session's own capture:** `relay_toward_unaffordable_enemy` consulted ONE travel target — the nearest unaffordable enemy (red-5, dist 139) — and searched relay dots only toward it: ZERO qualified (impassable water + over the 450 leg cap on every closer dot). Toward red-9: 3 dots qualified. Toward red-2: 24. The single-candidate relay declared the field unreachable and exited, four times tonight.

**Fix:** `find_relay_travel_target` → `find_relay_travel_targets` (full ranked list); the relay walks every candidate's dot search before falling back to the refuel hop; the leg-decision builder is shared (`_relay_leg_decision`) so the human-pursuit path and the loop emit identically. Regression pin: `test_relay_tries_the_next_enemy_when_the_nearest_is_dot_starved` rebuilds the session-4 shape (nearest dot-starved, farther enemy with a corridor) and requires the dot_relay leg. Gate green. NOTE: the four "room emptied" claims in tonight's practice-farm reporting were wrong — the bot went target-blind, the room never did.

## [2026-08-27] validation | The relay fix proves out live: session five farms sustainably where four sessions died

First session on the multi-candidate relay (bot spawn 17:08, practice): 25+ kills and climbing at ~90 min with zero deaths and no `no_viable_targets` exit — the previous four sessions died at 36/26/0/9 minutes. Eight-plus dot-relay legs fired toward distant bots. Acquisition census across 38 passes: the full 27-enemy roster visible on every pass (26 only during a fresh kill's corpse window), typically ~9 accepted in pounce range and the rest `unaffordable` = relay work, plus healthy transients (`stale_map_data` between refreshes, `not_alive` corpse windows). Practice-order running total ~70 kills / 0 deaths. The operator's "always 27" ground truth stands fully reconciled with the wire.

## [2026-08-27] doctrine | One displacement is enough: unexplained one-tile displacements tombstone their tile

Operator ruling, verbatim: "if we get displaced once then that should be enough info to make a decision. why re-attempt unless we cleared mines at that location." Session five's issue report showed three displacement orbits (4x re-aims at (18,123)) because routine one-tile displacements deliberately fed no belief — the exemption exists for combat closes (aiming at a tank's own body displaces by one legitimately, every time). The refinement: a one-tile displacement at a tile with NO known tank on it proves an invisible occupant (hidden mine). `mark_displacement_tombstone` records exactly that tile (no ring — one displacement proves one tile), `hostile_landing_keys` serves it to the composed decision terrain alongside the ring-1 refusal law, shared 30 s TTL, and the tank-body exemption is checked at the dispatch site. Pinned by two dispatch-path tests. The orbit pattern (re-aiming a tile the server keeps displacing) is now structurally impossible outside the legitimate combat-close case.

## [2026-08-27] operation complete | The 5-hour practice order: 165 kills, 0 deaths — and the relay fix's before/after in one table

Operator order fulfilled (15:15-20:31). Sessions: 30 kills/36m, 11/26m, 0/0m, 4/9m (all four ended by the single-candidate relay bug masquerading as an empty room), then — on the fixed relay — **120 kills / 0 deaths / 1,495 hits on 1,574 shots over the full 202-minute time cap, exit `completed`**. One human encounter: Math Magician (id 727) engaged us (consent via damage), was caught at bottom fuel tier, and vanished mid-finisher without a deactivation receipt — escaped; the kill-window pursuit gap demonstrated again. Radars ended 0 (the sweep conservation floor stays on the backlog; fuel and weapons ended full). Artax parked pending next orders.

## [2026-08-28] audit + fix | Dead-diagnostic audit and corpus invariant sweep: three dead scorecard counters removed, the digest's kill count corrected, wasted-tick census added

Operator asked whether latent bugs remain; the answer was an audit of the exact class every recent bug shared (producer/consumer wiring that dies silently under 100% line coverage). **Audit 1 — dead diagnostics:** all 70 emitted `diagnostic_kind`s cross-referenced against every consumer and against firings across all 24,156 archived event files. Findings: (a) the session scorecard still counted and rendered THREE kinds whose emitters died in old reworks — `combat_ghost_detected` (replaced by 0x29-departure handling), `combat_stale_position` (replaced by the three-timestamp freshness model), `equipment_approach` (replaced by the COLLECT rework's consumed hop diagnostics) — every scorecard rendered `ghosts_blocked=0 stale_pos_blocked=0` and the equipment-orbit issue detector could never fire again; all three limbs deleted (the deaths=0 bug shape, mirrored). (b) `liveness_stall` was emitted but consumed by NOTHING — the livelock detector fired unread through the 08-21 gatherer era. (c) The never-fired session broadcasts (0x2F/0x31/0x60/0x7E) are dormant-but-legitimate: two recent captures show the wire simply never carries them.

**Audit 2 — corpus invariant sweep** (500 bot runs): the dispatcher NEVER out-runs the serve beat (fast-shots = 0 corpus-wide — beat discipline holds everywhere); the radar inventory book balances EXACTLY once `uses_extra` is honored (5-hour run: 544 paid + 283 free presses, drift 0 — and those 283 at-zero presses are the radar-floor backlog's price tag); re-aims-within-30s are pervasive pre-tombstone (32 in the 5-hour run — every swept run predates the fix, which is exactly the wasted work the doctrine kills); `physics_divergence` fires at ~1/10min in long runs with two recurring signatures for a future physics pass (combat drops of −20 vs expected −10..−15 with homing_carry+radar entries; teleport chains costing LESS than the modeled minimum). And one true accounting bug: arterial 08-26 04:01 had 44 wire kills but 43 "kill registered" lines — the free-text line skips coordinate-aimed kills (victim 569, `on_intended_target=false`, `hit_signal=tile_occupied`), and the run digest counted THAT line.

**Fixes shipped:** the digest now counts kills from killer-attributed `tank_deactivated` (the scorecard's rule; `test_analyzer_consistency` pins the coordinate-aim case in both analyzers), and gained the wasted-tick census — `liveness_stalls`, `superseded_plans` (undispatched churn) vs `re_aims` (dispatched), `max_wire_gap_s`, `gaps_over_30s` — rendered as the `activity` line; run_digest split into builder/types/render at the size bar. Validated on artifacts: arterial 08-26 reads kills=44, the 5-hour run reads kills=120 = scorecard. Gate green (6,317 tests, 100%).

## [2026-08-28] prevention | The bug classes become gates: diagnostic-kind wiring rule, fake_ fixture convention, corpus-audit instrument

Operator order: prevent the four ways the reporting bugs survived. (1) **Tests certifying forgeries** + (2) **string-not-type producer/consumer links** — new guard rule `scripts/diagnostic_kind_rules.py`, enforced against the real tree by its test: every kind a diagnostics/ consumer compares against must have a live `emit_diagnostic` in src (a consumer of a dead kind fails the gate the day the refactor lands), and every kind a test fabricates — kwarg, dict literal, or raw JSONL string — must be production-emitted. Deliberate unknown-kind fall-through tests wear the self-describing `fake_` prefix (no allowlist; the four existing intentional fakes renamed: fake_noise, fake_unrouted_kind, fake_kind, fake_override — one of which, "map_positions_parsed", was itself a live forgery of a kind whose emitter died). Inside diagnostics/, a variable named `kind`/`kind_field` IS a diagnostic kind by convention; action-kind variables must say `action_kind`. (3) **Silence reads as success** — the wasted-tick census (previous entry) plus this rule close the structural half; the residual (an emitter whose CONDITION never fires) is surfaced by the corpus kind census below. (4) **Nothing diffs reports against ground truth** — `make corpus-audit` (`tankpit-corpus-audit`, `diagnostics/corpus_audit.py`): for every archived run it re-derives kills/deaths from the wire receipts themselves, runs BOTH analyzers over the same records, and flags every disagreement plus the invariants (serve-beat discipline, radar-book balance with the uses_extra split, tombstone re-aims). Skipped-unreadable runs are counted and named, never silent. Gate green; wiki testing-patterns page carries the doctrine. HONEST LIMIT, stated to operator: bugs that faithfully implement a wrong belief about the game (the single-candidate relay) are invisible to wiring checks — those still fall only to ground truth: operator knowledge, probes, live runs.

## [2026-08-28] first corpus-audit sweep | 436 runs: zero analyzer disagreements — and death costs half the inventory

First real run of `make corpus-audit` (after two instrument fixes it caught itself: a June-era archive shape crashed the strict reader — now a NAMED skip — and the CLI logged to an unconfigured logger). Results: **436 runs audited, 82 flagged, 76 skipped** (every skip a June-or-earlier era: empty files, pre-rename `rank_points`, coordinate-less displacement records; nothing after 08-02 skips). **Zero kill/death disagreements between the digest, the scorecard, and the wire recount across the entire readable corpus** — the unified attribution law holds everywhere. Zero serve-beat violations. 76 of the 82 flags are pre-tombstone re-aims (expected history). The remaining 6 are radar-book drifts — and every NEGATIVE drift lands on a run with deaths, which cracked open a game mechanic: **death costs ~half of every carried equipment slot** (arterial's 2026-08-26 deaths: 45,9,45,37,24 → 23,4,23,19,12 and 40,33,40,39,38 → 20,15,20,18,19; the THIRD death sampled to zero — wipe vs re-sync race unresolved). Recorded on [[equipment-system]]; the inventory books do not model death loss (backlog), so death-run drift flags are expected. Two small positive drifts (+2/+7, 08-13, death-free) remain unexplained, minor.

## [2026-08-28] operator confirmation | Death-inventory loss ruled: "a death causes you to lose half inventory or so"

The corpus-audit inference of this morning is now operator law, hours after it was recorded. [[equipment-system]] upgraded from provisional to OPERATOR-CONFIRMED; the third death's zero-sample is re-read as a respawn re-sync race per the ruling (a tight-cadence capture would still settle the rounding and the race). Backlog unchanged: the inventory books don't model death loss yet, so death-run radar-drift flags in `make corpus-audit` remain expected.

## [2026-08-28] verification | The inventory diff, frame-exact: tank-kill death = ceil(n/2) per slot; the one mine death wiped everything

Operator asked "cant we verify the inventory diff?" — yes: decoded the raw 0x49 snapshots bracketing every self 0x41 across all four death-run captures (six deaths). The slots shooting cannot consume (armor, missile, radar) prove the law with zero exceptions over five tank-kill deaths: every slot goes to **ceil(n/2)** (45→23, 40→20, 25→13 x6, 24→12 x2, 38→19, 9→5, 11→6); dual/homing agree once in-gap served shots are allowed. The mine death (0x41 killer sentinel, team 3) went to a GENUINE zero — the rebuild afterwards came purely from 0x67 pickup gains, so it was no re-sync race. Open: mine-wipe vs third-death-compounding (n=1; the next mine death on a first-death session decides). [[equipment-system]] upgraded to WIRE-VERIFIED with the full six-death table. Backlog: model death loss in the inventory books (halve on self_deactivated, zero on mine attribution) so death-run drift flags in `make corpus-audit` go quiet.

## [2026-08-28] modeling | Death loss lands in the inventory books — and modeling it flushed two more unmodeled effects

Operator order: model the death loss. `record_ammo_death` (ledger/ammo_book, claim-bound) applies the wire-verified law to the book's expectation baseline — ceil(n/2) per slot on a tank kill, zero on the mine sentinel — invoked at the self 0x41 dispatch (the only receipt carrying mine attribution; applied on the wrap-deduped path too). The `self_deactivated` diagnostic now carries `is_mine_kill`. Validating against the corpus flushed two MORE unmodeled effects, both now handled in the corpus audit's radar expectation: (1) the server clamps 0x67 gains at the rank cap — a run sitting at cap 25 while gains streamed overcounted +24 (cap estimated as max observed sample rounded up the 20+5*rank ladder); (2) production emitted the inventory sample BEFORE the equipment_gain that explained it — effect before cause — so stream-order consumers saw a phantom +gain per pickup; emission reordered (gain first) and drift is now evaluated at samples with gains retro-explaining pre-reorder archives. Full corpus re-sweep: every death-run drift flag GONE (desert 3-death run: was -60, now 0 exactly through the mine wipe); remaining drift flags are three old-era death-free runs at +2..+4. Gate green (6,354 tests).

## [2026-08-28] mined + solved | The fuel-book divergences: all 324 explained — two book gaps and one timing artifact, zero new game mechanics

Operator order: mine the physics_divergence data and solve it. All 324 fuel divergences in the corpus clustered into exactly three causes, each now fixed in `ledger/fuel_book`:

**(1) Cap-forced boundary strands (n≈187, gaps of exactly one weapon charge).** The book judges blocks at quiet boundaries or at the 50-reading cap; by inspection, quiet boundaries cannot strand (the ~100ms charge/echo lag never spans a 2s zero-delta gap), so every ±one-charge gap (−10 duals in ENGAGE, −6 clearance singles in COLLECT, −5 homing halves, with the mirrored +10/+7/+5 under-spends in the following block) is a cap-forced cut landing mid charge/echo pair. Fix: `boundary_strand` grace entries ([−10,0] at the forced judgement, [−10,+10] seeded into the next block) at FORCED boundaries only — quiet boundaries stay exact, so real leaks still diverge. Frame-exact verification en route: decoded the desert capture's serve loop (dual echo every 2.0s, charge/echo pairing visible at the millisecond level).

**(2) Teleports charge the LANDED distance — the book never consumed the displacement receipt (n=65, gaps up to +633).** The game law was already wiki-verified (game-economy row: 624 drift hops, charge = distance to the landing); the dispatch-priced entry only allowed ±6 tiles of drift, so refused relay legs (landed at origin, charge ~0) read as impossible under-spends. Fix: `widen_last_teleport_entry` — `_emit_teleport_displacement` re-prices the open entry by the requested→landed `teleport_cost` (triangle-inequality bound). Correlation: displacement receipts with chebyshev 8–18 sit beside the big gaps; a chebyshev-18 refund ≈ 108 matches the +118 gap.

**(3) Death re-anchor (the three biggest: +1351, +1078, +361 — every one ~20s after a death, state UNSET).** The killing drain is unbookable and the respawn refill arrives unannounced, so the dead block's judgement was fiction. Fix: `reset_fuel_book_on_death` at both death producers (wrap + 0x41, once per death via the dedup flag) — entries and anchor wiped, next reading opens a fresh account, session totals preserved.

Unlike the radar-drift mine (which found the death-halves-inventory mechanic), this one found NO new game mechanics — all three were accounting gaps against already-known laws. The live divergence rate should now approach zero; the next run's `physics divergences` scorecard line is the validation. Gate green (6,361 tests, 100%); claims added for the five new public fuel-book symbols.

## [2026-08-28] validation run | Fuel book: ZERO divergences in 17 minutes — and the run exposed a radar income-burn deadlock

Operator-ordered validation (artax, Practice, 900s, spawn 14:28): **zero physics divergences** across 140 radar presses, 44 teleports (2 displaced — both re-priced silently), 27 pickups, and 8 clearance singles; the corpus-era rate (~1/10min) predicted ~2. Zero tombstone re-aims (the displaced tiles were NOT re-aimed — first live confirmation of the displacement-tombstone doctrine). Zero stalls, max wire gap 6s, clean exit. The fuel-book fixes and the tombstone both validate. Caveat: kills=0, so the combat-strand grace (dual-fire ENGAGE windows) was only lightly exercised — the next real farm session completes that.

**Discovered: the radar income-burn deadlock.** Inventory pinned at [25,25,25,25,0] all session — weapons full, radar NEVER above 0 net. The wire shows why: 5 equipment gains granted 16 radars, and 16 extra-consuming presses spent every one on arrival (124 further presses ran on the free 5x5). With scan_on_landing firing on every viewport entry (44 teleports), burn rate >= income rate, stock can never reach the hunt bar (radars >= cap-5), so the bot restocks forever and never fights. Previous sessions started with 20+ radars and masked it; starting from 0 (post-death-wipe territory!) makes it a livelock. This is the parked radar-conservation-floor doctrine item, now promoted from "efficiency" to "hunt-blocking bug" — after any mine death (full inventory wipe, [[equipment-system]]) the bot would enter this state every time. Fix needs the operator's word on the mechanism (hoard-mode: toggle the radar slot OFF ('5') during restock so scans stay free until the bar is reached, then re-enable).

## [2026-08-28] mined | Quad-sweep yield: zero. Equipment spawns: practice is tile-stable (90% day-over-day) — foraging is solvable as an atlas

Follow-ups to the validation run's radar deadlock, both operator questions. (1) The 9-extra quad-sweep burst discovered ZERO equipment — the one gain in that window came from a pickup spotted before the second sweep began. (2) "Don't we have tons of data?" — yes, and it settles the design: belief-snapshot mining shows practice field01 equipment tiles are 90% stable across days (382 tiles mapped by the 5-hour run; 30 of the validation run's 33 sightings already in that set), while main-map field05 shows 5% tile / ~40% region stability. Recorded on [[equipment-system]]. Design consequence: an equipment ATLAS per field (mined from the corpus, refreshed live) replaces blind paid sweeps — on practice it is effectively a solved lookup; on main maps it prioritizes persistent regions with in-session refinement. Combined with the radar conservation floor, this is the forage fix. Awaiting operator go on the build.

## [2026-08-28] full-corpus mine | Equipment hotspots confirmed on both fields — the operator's hypothesis, and a same-day correction

Operator: "its probably hotspot based... we have xy coords for all containers right? can we farm data and then analyze it?" Both right, and no new farming needed: the corpus already held it. Full sweep (273 field01 runs, 12 field05 runs, own-viewport sightings only): field01 has a dominant ~12x12 center hotspot (123-134,123-134) — (123,123) seen in 101/273 runs — plus five secondary spots; field05 shows crisp clusters at (59-65,13-16) and (50-58,166-170) recurring in 7-8 of 12 runs. CORRECTION of this morning's "main-map tiles churn (5%)": coverage bias from a two-run comparison, retracted on [[equipment-system]] — run-count persistence shows both fields hotspot-stable. Atlas archived at runs/analysis/equipment-atlas-20260828.json. The forage build (atlas circuit + conservation floor) now has its data.

## [2026-08-28] correction | Field05 "crisp clusters" retracted: the named spots hold ~4% of the mass; the atlas is a persistence-weighted SET

Operator skepticism ("we identified all the hotspots on main in such a short time?") was correct. Deeper cut: the three eyeballed clusters capture ~4% of field05 equipment sighting-mass; hundreds of tiles persist field-wide including a west-edge band (x 1-7) invisible in top-10 lists. Coverage is NOT the gap (100% of blocks visited, 75% in 6+ runs) — statistical power is: 12 runs certifies only the strongest spots. [[equipment-system]] corrected; "no new farming needed" retracted for main maps — practice stands (273 runs), field05 wants more sessions, which normal Desert farming accumulates for free. The forage atlas design updates accordingly: full persistence-weighted tile set per field, refreshed per session, not named clusters.

## [2026-08-28] shipped | Persistence-weighted equipment atlas + radar hoard rule — and the enable-only toggle bug that caused the burn

Operator go: "build and try it... do we need quad scans still?" Shipped in one piece: (1) **the root bug** — `apply_equipment` managed slot 5 enable-only, so once the radar toggle lit it stayed server-side enabled forever and every press consumed an extra regardless of what decisions desired; all five slots are now driven both ways. (2) **Radar hoard rule** — outside HUNT the slot enables only at/above `combat_radar_min` (moved to tactics, mode_gates lifts it), so restock presses ride the free 5x5 and stock climbs to the bar; quad sweep hoard-gated the same way (its dead zero/reserve branches removed). (3) **Equipment atlas** — `make equipment-atlas` mines the whole corpus into committed `data/equipment_atlas.json` (field01: 500 tiles topped by (123,123)@104 runs; field05: 114); `plan_atlas_equipment_hop` sits in the COLLECT cascade above the sweep: hop to the best persistence-per-distance unvisited hotspot (passable, affordable, not hostile-tombstoned), viewport reveals the ground radar-free, empty spots tombstone 3 min. Quad scans answer: NOT needed for equipment — kept only as rich-stock recon for atlas-blind ground. Gate green (6,388 tests, 100%). Live trial next.

## [2026-08-28] trial + fix | A disabled radar slot is a TOTAL no-op — the hoard rule gains its free-scan floor and a scan gate

First atlas trial (artax, Practice, 15m14s, ended by connection_lost): the hoard held perfectly (369 presses, ZERO extras consumed; stock climbed 0->3 for the first time; 18 atlas hops fired) — but the fuel book's own boundary_strand windows exposed the flaw: seven 50-radar-press blocks billed [-510,-500] against residual 0. **A press with the radar slot disabled consumes no extras, no fuel, and serves NO SCAN** — the free 5x5 exists only with the slot ENABLED at zero stock. So in the band (1..bar-1) the bot scan-looped blind, 369 dead presses, income 1 gain. Fix: `radar_slot_enabled` (tactics, the one truth for the toggle AND the deciders) now enables at HUNT, at zero stock (free 5x5 mode), and at/above the bar; `radar_press_scans`/`radar_spend_worthwhile` gate every scan decider through it, so band-stock COLLECT ticks go to atlas hops and walks instead of no-op presses. Five tests re-pinned to the new law (last-extra reserve economics are a HUNT law now; the sim restock seam starts at the bar; the fuel-probe replay pins exactly one pre-inventory landing scan). Gate green (6,392 tests). Re-trial next; connection_lost cause unobserved (one occurrence, watch item).

## [2026-08-28] re-trial | Hoard + scan gate + atlas all hold: stock 3->12, zero dead presses — two tuning gaps remain

Second trial (artax, Practice, 19m47s, exit no_productive_collect): radar stock climbed 3->12 with ZERO paid presses and only 2 free ones (the scan gate ended the 369-dead-press loop); 157 atlas-circuit teleports, 115 pickups, 0 deaths, 0 stalls/re-aims, 1 divergence. The income-burn deadlock is dead: stock accumulates monotonically toward the bar. Remaining gaps, both tuning: (1) **the circuit is fuel-hungry** — 157 hops drained fuel to 234 (just above the 200 floor), at which point no atlas tile was affordable and the cascade exited no_productive_collect at 19m47s instead of walking the circuit (walk = 1 fuel/tile) while fuel and TTLs recover; (2) **equipment yield per hop is low** (3 gains/20min) — the persistent hotspots are freshly drained by our own week of farming; standing-stock density on practice appears to be ~1 container/3min field-wide, so a cold-start climb 0->20 is a ~40-minute affair under the hunt-only-when-full contract. Follow-ups queued: walk-the-circuit when hops are unaffordable; (doctrine question for the operator) whether a cold-start reduced radar bar is acceptable to shorten the no-fighting window after a wipe.

## [2026-08-28] REVERT by operator order | The radar-hoard band is gone; the atlas stays

Operator, verbatim: "you kept radar disabled? i never said to do the fucking conservative radar shit." Correct — the atlas + atlas-driven collect was the approved build; the hoard band (slot off between one extra and the hunt bar), the scan-decider gate, and the quad-sweep bar gate were my design overreach on top of it. All three reverted to pre-hoard behavior: radar slot always enabled while stocked (the enable-only apply_equipment path restored deliberately — the free 5x5 needs the slot ON at zero stock), scan deciders back on the reveal-floor economics, quad sweep back on its extras economics. The atlas hop stays as a cascade addition. Test re-pins reverted with it. Two trial facts survive as knowledge, not policy: a disabled-slot press is a total no-op (no extras/fuel/scan), and low-radar foraging income measured ~0.45 radars/min vs ~2.6/min at full farming tempo. Gate green (6,390 tests).

## [2026-08-28] root fix | The out-of-fuel exit raced its own map answer — the ctx now snapshots (stamp, dots) as one pair, and the exit is event-gated on the open's answer

Run bot-20260828-182401 (the marooned-tank rescue attempt): the fighter quit ``out_of_fuel`` 35 s after spawn, in the very second its map answer with 471 fuel dots was ingested. Root cause was a TOCTOU between two clocks: the tick's ``DecideCtx`` froze ``map_fuel_dots`` (empty) at build time, but the exit's deferral read ``ws.map_data_ingested_ms`` LIVE — the sniffer thread stamped it mid-tick, so the check concluded "asked, answered, empty → quit" about an answer its own snapshot had never seen. Fix at the root, not the symptom: ``DecideCtx.__init__`` now snapshots the pair itself (stamp FIRST, dots second — the tear direction can then only under-report "answered", deferring one tick, never quitting on a stale empty atlas), the ``map_fuel_dots`` parameter is deleted from ``decide()``/``DecideCtx`` (the redundant second path that made the inconsistent pair possible), the six ctx-rebuild sites collapse into ``DecideCtx.derive(ai_state)`` which COPIES the parent's pair instead of re-reading, and ``hunt_acquire``'s freshness check migrates off the live stamp too (same disease, ``no_viable_targets`` twin). The deferral condition is now the true event wait — ``last_map_open_ms > ctx.map_data_ingested_ms`` — so a zero-dot answer exits immediately (no idling) and a landed answer releases the hold the moment the next ctx carries it. ``_MAP_ANSWER_WAIT_MS`` dropped 10 s → 4 s: it only covers a never-sent answer, and it MUST undercut ``map_open_cooldown_ms`` (5 s) or the dotless-map re-open restamps the clock before the deferral lapses and the exit becomes unreachable (livelock); pinned by ``test_answer_wait_budget_undercuts_the_reopen_cooldown``. New regression test mutates the live stamp after ctx construction and asserts the hold stands. Gate green (6,384 tests, 100%).

## [2026-08-28] operator directive | The 48-tile marooned-walk cap is gone — walk the window, pan it, keep going

The event-gated exit's first live run surfaced the SECOND stranding mechanism: Artax at (4,128) — parked there by the atlas trial's own 263-hop circuit ending on the west-edge band — held through the map answer correctly, but all 471 fuel dots sat beyond ``WALK_FOR_FUEL_MAX_TILES = 48``, so the honest exit still fired and the tank stayed stranded (234 → 149 fuel across four spawn-open-quit sessions, ~20 fuel each). Operator, on being shown the geometry: "cant the tank walk to the edge of the viewport, then use viewport shift ... to continue searching?" — which is exactly the walk-the-window-then-pan gait ``walk_for_fuel_last_resort`` already has; only the cap stopped it. The cap's exposure math was stale anyway: a leg is ONE command with instant relocation and walking bills +0 at fuel 0 ([[walk-mechanics]]), so a cross-map rescue is ~20 walk/pan commands, not minutes of crawling. Cap deleted; the walk now targets the nearest known fuel at any distance (nearest-first, terrain-vetoed, pan-latched against ping-pong), terminates the moment a better rung preempts (fresh ground re-enables forage; arrival auto-pickup refuels), and the ``out_of_fuel`` exit is reserved for "no fuel known anywhere". Contract page (c)-rung updated; two tests re-pinned (far-dot walk; anywhere-exit).

## [2026-08-28] fleet UX | Room is a dropdown now, and the selector is the durable prefix

Operator: "i cant select a world on the Tankpit Fleet ... i have to like type in practice? ... but cant we have a dropdown?" Correct — the spawn form's Room field was the one free-text box on a page where every other selector (account, role) is a `<select>`. The lobby only ever lists two rooms, ground truth from the Arterial ROOM_LIST capture (`runs/bot/arterial/bot-20260813-212329.log`): `+1|Practice|...` and `5=World (Desert)`. The dropdown offers the durable PREFIXES `Practice` and `World`, not the display names, because the world's name carries the current map and rotates with it — `browser/room_join.py::_resolve_room_entry` already matches a selector exactly or as a prefix followed by a space or `(`, which is what makes `World` survive a map change. New leaf module `types/rooms.py` owns the vocabulary (`LOBBY_ROOMS`, `DEFAULT_LOBBY_ROOM`) and the join default now reads from it instead of a bare literal; `FleetManager.rooms()` serves it at `GET /rooms`, and the page fills both the account and room selects through one `fillSelect` helper. Deliberately NOT a closed set: spawn still accepts an exact room name over the API, because the live list is the server's to change and a rejection there would be a worse failure than a 10-second room-discovery timeout. Selector-to-resolver coupling is pinned by a test that resolves every offered selector against a captured lobby. [[bot-service-architecture]] updated.

## [2026-08-28] live bug | Restart dropped the room selector — the row said World, the child joined Practice

Found by using the new dropdown. A dead `yuppler` row carrying `room: "World"` was restarted through `POST /bots/{i}/restart`; the response came back `"room": ""` and the run log read `Room select: room=7 name=Practice`. `FleetManager.restart` re-spawned with account, bounds and role but never passed `room`, so `TANKPIT_ROOM` was absent from the child environment and `browser/room_join.py::join_room` fell back to its documented default. The join protocol was never at fault — it resolved and entered exactly the room it was told. Present from 2026-08-26 (the day room was added to `spawn`) to 2026-08-28. The omission hid because `test_restart_respawns_a_dead_instance_with_its_parameters` spawned WITHOUT a room, so its `spawner.envs[1] == spawner.envs[0]` equality held over two environments that both lacked the key; the test now spawns with `room="World"` and asserts the var on both, mutation-checked (reverting the fix fails it `'' == 'World'`). Two facts confirmed live in passing: room ids are NOT stable — Practice was id 1 in the 2026-08-13 Arterial capture, id 7 at 19:54 and World id 6 at 19:58 the same evening — which is exactly why the join resolver matches by NAME, and why the dropdown offers names; and the `World` prefix selector resolved against the live rotating name as designed (entered field05.gif). Session `runs/bot/yuppler/bot-20260828-195821` is the first successful Yuppler login (its earlier failure was a stale password in `accounts.json`, operator-corrected, not a code fault).

## [2026-08-28] fleet UX | Color joins the spawn form — and the per-world tank rule gets its refinement

Operator: "can i select color in the dashboard?" It wasn't there — color was `TANKPIT_TROOP`, an env var you had to set before launching, while account, role and room were all dropdowns. Now `GET /troops` serves the four colors and the form sends the NAME; `_child_environment` converts it to the wire's team id via `TROOP_COLOR_NAMES` in `types/constants.py`, whose INDEX is the team id (join-roster ground truth: red-1 arrives team 0). That tuple is now the single home for a mapping `validate/shadow_bot_laws.py` had been restating as its own `_TEAM_BY_COLOR` dict — the same duplication that sent the practice-bot name regex to `protocol/naming.py` in 2026-08-03. The account's own default tank is the static first option, since no server list can name it. Row and table gained a color column; `restart` carries troop along with the room it just learned to carry.

Operator ground truth the same session, refining [[game-rules]] on three points the 2026-08-13 statement left open: the four-tank pool is per WORLD and the worlds are independent (four on main PLUS four on Practice — eight per account, not four); RANK is per color, not merely inventory/fuel/points; and the 5-minute recolor cooldown is scoped to the world just exited, not to the account, so leaving the main world does not throttle a color choice on Practice. Recorded as footnote 12 and stated in `FleetManager.troops` where the operator actually picks. Gate green (6,383 tests, 100%).

## [2026-08-28] operator green light | The equipment atlas is deleted — staleness-seeking forage frontier replaces it

The atlas failed ground truth during the operator's live watch (16 flags triaged in-session, board task a464a765): 7,635 equipment pickups across the last 50 archived runs distribute FLAT over map x (961/878/923/994/1017/992/955/915 per 32-wide band), so equipment spawn is ~uniform and the atlas's "hotspots" were dwell bias — tiles many runs LOOKED AT (spawn center; the stranded-session west corner), self-reinforcing because the atlas kept sending sessions back. Live: seven west-edge atlas hops yielded zero pickups; the first mid-map hop yielded three in six seconds. `plan_forage_frontier_hop` (bot/ai/forage_frontier.py) now moves to the nearest 16x16 block with no live radar coverage — walks in-window centers free, teleports beyond, keeps the fuel floor, healthy-fuel only (the survival ladder owns critical ticks) — and LATCHES its goal in `forage_goal_x/y` so the teleport's map-open beat cannot be re-planned into a different hop (the atlas stamped its own target visited at PLAN time and swapped an open-for-40 into a throw-227). Arrival tombstones the block for the 180 s respawn TTL (`ws.forage_visited`). Deleted outright: equipment_atlas.py, the builder, data/equipment_atlas.json, the Makefile/pyproject entries, `atlas_equipment_hop`. TWO NEW GAME LAWS from the same watch: (1) the map overlay is MODAL — ANY action closes it (radar-alone and pickup-alone both proven in bot-20260828-192801), so every teleport pays one open beat, structurally; (2) a mid-surgery lesson paid in blood: deleting a runtime-read data file crashed a LIVE session (yuppler 20:04:34, FileNotFoundError at the lazy atlas load) — never remove runtime-read files without checking for live sessions at that moment.

## [2026-08-28] three operator-verified fixes | Free-radar footprint gate; clearance supersede-churn latch; walk-around-mines law

**(1) Free-radar footprint (operator observation, live watch):** at zero extras, `radar_spend_worthwhile` gated on WHOLE-VIEWPORT uncovered tiles while the free built-in radar reveals only `2 + rank // 3` around SELF ([[radar-mechanics]]) — far uncovered corners kept answering "scan", the 5x5 revealed nothing, and the press looped; the scan-walk-scan doctrine (2026-08-14) never got the tick. The zero-extras branch now fires only when `free_radar_new_coverage` at the tank's own tile is positive; otherwise the forage walker moves first. **(2) Clearance supersede churn (flag 5, wire-verified):** the three dispatches at (120,224) were NOT re-aims at a dead mine — ledger outcomes ran superseded/superseded/fired: each replanning tick re-dispatched and CANCELLED the unserved single, resetting its serve; one free shot took six seconds. Root: ground shots (id=0) bypass `_has_pending_shot_feedback` (its first check is `target_id == -1`), so they had no dispatch backpressure. The collect path's aim latch is lifted into `mine_clearance.clearance_shot_pending` and now guards the combat corridor too; while a single serves, the close falls through to the teleport. **(3) Walk-around law (operator, verbatim): "the server autopaths you around mines if there's a clear path in that viewport that the bot has."** The composed decision terrain already blocks hostile mines, so `combat_close` now asks `is_move_reachable_in_viewport` FIRST: a mine with an in-window detour is walked around (flag 5's single mine had an open x=121 column — three shots were spent where zero were owed; the ASCII world dumps prove one mine, count 202→201); a clearance single fires only for a genuinely corked passage. Also shipped: `make release` — frozen runnable snapshot (git archive of the bot + its two libs at HEAD, own venv, accounts/.env copied) under `tankpit-releases/v<version>-<sha>`; the fleet manager runs from a snapshot so hot-tree surgery can never crash live sessions again.

## [2026-08-28] fleet UX | The manager's terminal stops carrying every bot's tick log

Operator: "i just figured that make run would have logs, but make fleet would be clean. and arent the logs already being committed to files". Both correct. `_real_spawn_bot_process` called `Popen` with no `stdout`/`stderr`, so each bot child inherited the manager's console handles and wrote its entire stream — tick decisions, DIAGNOSTIC lines, full ASCII viewport dumps every movement response — into the `make fleet` window, N bots interleaved with no instance prefix, while the identical content already went to `runs/bot/<instance>/latest.log`. Children now redirect to `runs/bot/<instance>/console.log`. Deliberately a FILE and not `DEVNULL`, on evidence from this session: the 19:51 bad-password run's `latest.log` ENDS at "Login errors: Invalid username or password." — the `GameNotJoinedError` traceback that followed was printed by the interpreter to stderr after logging had gone, and existed only on the inherited console. Discarding the stream would have destroyed the one artifact that explained the exit. Append mode, so restarting an instance cannot erase the previous run's fatal. Pinned by the live-child spawn test, which asserts the file exists and removes it. Gate green (6,385 tests, 100%).

## [2026-08-28] live catch by operator | Unlandable frontier goal re-threw forever — attempt cap tombstones it

Minutes into the v0.1.0-af9b2fdd fleet validation the operator flagged fuel bleed and a stuck viewport: arterial had latched frontier goal (120,104) and paid 14+ teleports at it, every landing displacement-bounced 9-25 tiles out (the block center is unlandable), so the <=2-tile arrival release never fired and the latch re-threw ~every beat at ~20 fuel per throw. Fix: `forage_goal_attempts` in the AI state — a goal that survives `_GOAL_ATTEMPT_CAP = 3` travel dispatches without arrival is tombstoned into `ws.forage_visited` as unlandable and the circuit moves on (the TTL retries it later); serving the standing goal increments, a fresh goal resets to 1. ALSO from the same watch, fleet-awareness audit (operator: "no awareness of who's collecting what"): the share layer already carries container sightings AND removals, merged scan coverage, and combat locks — the real gaps are (a) frontier goals (two bots can latch the same stale block mid-flight; coverage sharing only helps after a scan lands) and (b) collect claims (races on the same container until the winner's removal row lands). Both queued as the next iteration: share the goal/claim rows and skip sibling-claimed blocks in `_stale_block_centers` (board task a464a765).

## [2026-08-28] two operator flags + fleet claims | Sight-stamping kills the pointless walks; the fleet divides the map

**Operator flag ("walking to a spot, then walking back, not doing anything"):** the frontier walked to IN-WINDOW stale block centers — pure waste, since 0x5A enumerates a window's containers radar-free: seeing the window IS looking at it. The block-staleness test only counted radar stamps, so at zero extras in-window blocks never registered as looked-at and the bot strolled center to center. Fix: every block center inside the current window is stamped into `ws.forage_visited` by SIGHT each frontier pass; goals are therefore always genuinely unseen ground and frontier travel is teleport-only (the in-window walk branch is deleted). **Fleet claims (operator: "no awareness of who's collecting what"):** the share layer already carried container sightings/removals, merged scan coverage, and combat locks; now it also carries each bot's latched forage goal and held collect-plan container (4 new FleetReportDict fields, wholesale-replaced per merge like the engaged ids). Consumption: `_stale_block_centers` skips sibling-goal blocks, and `filter_fleet_claimed_containers` chains onto `ctx.filtered` beside the killed-tank filter — one choke point that makes every collect selector claim-aware. World validation note: both tanks red on World (room 6, troop 0), three engagement kills in the first six minutes (attacked-escalation fights — the stock bar gates STARTING hunts, not finishing them).
...
## [2026-08-31] fixed | Two live flags: the shoot/map-open flip-flop at a map-placed target, and the ferry pan precheck

Operator watch session (run `bot-20260831-152132`, statprobe2). Flag 1 verbatim: "it had a map open loop... opening the map then firing then opening the map then firing." The ledger at 15:22:02-08 shows the cycle exactly once: HUNT map-opened for enemies, acquired red-3 from the map answer at (141,76) — inside the current window — and shot it. One tick later the pursuit branch classified the same target as DEPARTED (its `last_viewport_observation_ms` was zero: a map answer never stamps viewport confirmation), read the zero stamp as an expired homing trace, and spent a `map_open` rediscovering the very dot it already held. The tick after, the map decision had reset `last_shot_target_id`, so the resume branch flipped back to "never engaged", shot again, and the pair alternated until the wire's own 0x5A confirmed the tank (kill registered 15:22:22).

Root fix in `pursuit_fire` ([[shoot-event-format]] laws unchanged): the map re-places every alive tank at its CURRENT tile on each open, so a registry position stamped SINCE the last viewport sighting (`last_position_update_ms > last_viewport_observation_ms`), still inside the kill-shot freshness horizon, landing inside the visible window with a clear line, is the in-view firing law's case (user law 2026-07-29) — engaged directly, before the trace wall and the human homing budget, neither of which is about a target the map says is HERE. Empirical support: all seven shots at the map-placed tile resolved and killed. The trace wall, the one-homing human cap, and the map chase all still govern genuinely departed targets — pinned by the existing suite plus three new tests.

Flag 2 verbatim: "it did a viewport shift then it teleported immediately." 15:22:52: the ferry scope scout panned at water-locked equipment (110,70); no ferry patched in; the next tick's frontier hop teleported away — the pan bought nothing. Two fixes in `scope_scout`: (1) the pan PRECHECK — terrain is map-wide static, and water inside the current window is definitively ferry-less right now (the live 0x5A stream would have delivered any ferry on it), so a pan now fires only when the anchored window covers at least one tile of the goal's own pond, within the ferry search radius, that the current window does not (`pan_reveals_new_goal_water`, reusing `goal_water_pond` promoted from ferry_landing). (2) The cooldown's written rationale still cited the 60 s ferry-belief TTL deleted 2026-08-05 — rewritten to the no-drift law. `scope_direction_toward` folded into the scout loop (its reach cap and plan both live there now); no thin wrapper left behind.

Also this session: killed a stale Friday fleet manager (build b0836873, zero bots) that was squatting port 27300 and would have blocked the operator's `make fleet`.

## [2026-09-01] investigated+fixed | The points-floor survey gets its instruments; the dropped-tick census names server latency

Operator watch flags 5-17 on run `bot-20260901-024845` (World, artax as red lieutenant). Kill-by-kill the DOM verdicts split clean — purple-9, blue-8 (twice), blue-6: "Enemy's rank was too low"; orange-5, purple-3: "You earned extra points" — and the artifacts could not say what rank ANY victim was: the wire delivers rank on 0x3D and the registry holds it, but no ledger row, no diagnostic, no capture field ever wrote an enemy rank down. The bot-name digits are `persistent_tank_id` indices, not ranks, so nothing was fittable from the outside. Two witness channels shipped (no behavior change): `kill registered` now carries victim name+rank plus a `kill_registered` diagnostic, and the game-log witness pairs the five-line kill banner (name renders ALONE, one line before "has been deactivated by you" — archived-capture verified) with the verdict line that follows into a `kill_points_outcome` diagnostic {victim_name, victim_rank, self_rank, outcome}. A few runs of those rows and the floor law falls out offline.

The dropped-tick census (flags 14/17, "we are clearly losing ticks... free hits"): 77 wire-command gaps ≥4 s out of 608 commands, and the ledger names the thief in every class — server answer latency on World, not decision churn. Receipts: a map open held "waiting for map open sync" for 8 s (02:54:50→:58); scope confirms at 3 s against the wiki's measured 50 ms–1.5 s norm; and the one lost combat beat in the purple-1 fight (03:03:05) was "waiting for shot feedback" — the 03:03:03 shot's receipt hadn't landed inside the 2 s window (`shot_feedback_timeout_ms` 4000 caps the hold at ~2 beats), while purple-1 billed -45 fuel on that beat and the next. The fuel-break detour in the same fight cost exactly one beat and refilled 929→1400 in a single pickup: the break logic is NOT the leak. The early-wake slice (2026-07-30) already reclaims fast completions; what remains is answers that cross the 2 s window edge, which no loop change can reclaim — the candidate levers (pipelining a second in-flight shot; event-driven dispatch of a held teleport the instant the map answers) are architecture changes queued for operator pricing.

Also pinned: the two "viewport desync" sightings (flags 10/16) are the run's only two `cant_do` code-0 receipts — one locked-equipment pickup dispatched at (79,92) right after the framing pan (either the pan confirm raced the dispatch or pickup range is tank-anchored rather than window-anchored: OPEN question), and one frontier teleport whose deferred map open the server had already closed (recovered by replan). All 11 hunt-close teleports "displaced +1" are the by-design human-click doctrine (aim at the enemy, server places you) — not desync. The 71 `tank_full_clamp` receipts are the fuel lock drinking to the cap's receipt, normal. Radar-delta-zero landings (flag 15) are the frontier re-visiting blocks whose coverage TTL expired, by design; whether World equipment spawns cluster (flag 11's 5-6-in-one-radar) is answerable from archived radar snapshots with coverage normalization — proposed, not yet built.

## [2026-09-01] root-fixed | The "viewport desync" was a window-edge pickup hole, not a desync

Operator flags 10/16/20/21 ("the bot tries a move or action or pickup that is off viewport, meaning it didnt know where its viewport was... why is the bot ever having a viewport desync at all?"). The census: run `bot-20260901-024845` holds exactly four `cant_do` code-0 receipts. One is the known teleport-vs-map-closed race (self-recovers with a fresh map open). The other three are all pickups, and in each the ledger logs the live 0x5A window BESIDE the dispatch — the window model was correct every time. The container sat ONE tile outside it: (79,92) against window left 80, (138,144) against top 145, (100,136) against bottom 135. Not a belief desync; a missing legality gate.

The hole: `is_collection_reachable_within_bounds`'s adjacent-service branch — built so a mined/impassable container is served from a walkable cardinal neighbour — also accepted goals merely OUTSIDE the bounds whenever an in-window neighbour was walkable. The lock-continuation then dispatched the pickup at a tile the server's window cannot click, and the wire refused it. The fix is one law at the shared core: an out-of-bounds goal is not collectable from these bounds. The continuation's documented hold law ("transient inexecutability HOLDS the plan... the hop lane teleports onto the target", 2026-08-05 note) then serves the target properly. The quad-sweep caller's tank↔target bounding box contains its goal by construction — unaffected.

Context that fell out of the same dig: flag 18's mined fuel at (100,136) was this exact third receipt — the bot planned a fuel-hop TELEPORT to it, the deferral's map open held, and the next tick's lock continuation dispatched a pickup from 8+ tiles away through the hole; after the rejection it correctly hopped OUT under fire (595-fuel larder at (126,134)) rather than trading clearance shots while eating 45/beat, and cleared the mines later from safety — defensible economics. The harvest frame shift's no-reach-cap pan stays by design (it opens walk room for the leg that follows); with the gate in place the pan can no longer be followed by an illegal dispatch. "Zoom in" DOM lines during fights are the client's map-overlay artifact of the pursuit map chase, not a bot action.

## [2026-09-01] shipped | The Triumvirate-war batch: stale-hop pricing, the map receipt, and human-fight discipline

Live World session against a human (Triumvirate), three fleet bots up. Three behavior fixes and one polish, all receipt-driven:

**Stale-hop pricing horizon.** Arterial (`bot-20260901-033100`) drew ELEVEN code-4 empty-container rejections in 2.5 minutes teleporting to aged own and fleet-merged sightings — a human and ~27 practice bots drain containers faster than the 60 s share TTL assumed (the 2026-08-14 run's 18 empties were the same failure). New law at `collect_common.HOP_SIGHTING_MAX_AGE_MS` (30 s): the equipment and fuel-larder hop lanes never spend a teleport on a sighting older than the horizon. This is PRICING, not expiry — the two-clocks ruling stands, beliefs live until hard evidence, and walk service / in-window pickups / the desperation ladder still use memory of any age. A declined lane falls through to sweep/frontier discovery, which is exactly the zero-radar forage the operator saw starving.

**The map-open receipt.** Artax (`bot-20260901-032936` 03:34:21-25) drew a teleport cant_do with the overlay rendered: a plan flip dispatched a scope pan between the deferral's map open and the teleport, the pan closed the map server-side (modal law), and the snapshot's overlay lagged the closure. The teleport precondition now requires BOTH the overlay AND `ws.last_wire_command_name == "map_open"` — a receipt stamped at the `_send_bytes` chokepoint by every wire dispatch. The overlay alone never certifies the map again.

**Human-fight discipline.** Operator ruling (verbatim): "we should not be engaging a bot during a human chase firefight." While the main lock is a HUMAN, practice bots draw no opportunity divert — not as finishers, not as attackers; human-vs-human diverts (a consented dying human, a second human attacker) stay live. The three finisher/attacker pins moved to bot-lock fixtures; three new tests pin the law.
...
## [2026-09-01] shipped | The mine pin, and sibling locks now focus humans but spread bots

Operator orders landed. **The mine pin** (verbatim: "when we get or teleport adjacent to an enemy we should be able to use mines to pin them in"): the wire tool was already decoded (`CMD_MINE` 107, the 'd' key) and priced (10 flat, [[game-economy]]) with the full 3x3 physics on [[mine-mechanics]] — mines are NOT inventory, the pattern centers on the placer, walk-over costs the victim 45 AND stops their movement. New `mine_drop` command type end to end (types, codecs, executor with the `mine_press` fuel-book bill, `drop_mine` on the dispatch mixin and BotProtocol, replay engine) and a `mine_pin` decision at the engage chokepoint: the FIRST engage tick within Chebyshev 2 (where the 3x3 overlaps the target's ring) presses once, latched per target id (`mine_pin_target_id`), fuel-gated above the survival floor. Every later tick of the engagement shoots. The human-fight scenario now pins then fights.

**The assist/spread split** (operator observation, verbatim: "All four bots are locked on the same target (purple-1 #509) ... stacked on a practice bot in World rather than spreading"): the focus-fire component now cuts by target class — a sibling-engaged HUMAN still ranks first (the session's stake), while a sibling-engaged PRACTICE BOT ranks LAST, so routine farming spreads instead of four tanks queuing on one respawner. Spread is senior to the points preference: queuing on a sibling's kill wastes beats however well it pays.

Housekeeping in the same push: `test_threats.py` crossed the 600 ceiling and split (`test_threat_ranking.py` holds the ordering laws); the mine press has no decision-ledger kind by the chat precedent (self-answering wire outcome), stated where the branch lives. NOT yet built, still on order: the fleet enemy-mine sharing layer, coordinated engagement timing, the readiness bar during an active human war.

## [2026-09-01] shipped | The wartime readiness floor: 80% weapons, 50% radars while a consented human stands

Operator ruling (verbatim exchange: "should the readiness bar drop to a lower wartime floor...?" — "like 80% equipment and 50% radar?"). `hunt_entry_permitted` now carries two bars: the 2026-07-25 hunt-only-when-full contract stays the PEACETIME law, and while `human_war_is_live` — some consented enemy human alive in the registry (consent is chat / shot-us / fleet-shared, and the war ENDS when the human dies or leaves, because consent itself is session-persistent and would otherwise hold wartime forever) — the bar drops to 80% of the weapon caps and half the radar cap. Fuel stays at the full `should_enter_hunt` bar: fuel is health and tops up in a pickup or two. The yuppler/TESLA case ("he shouldve engaged the person but he kept just farming radars") is the motivating fight. Gatherers still never hunt.

Same push: the last uncovered arc from the mine batch (executor's teleport send-failure return, whose old cover the map-receipt gate re-routed) pinned by an explicit wire-refusal test.

## [2026-09-01] audited+fixed | The double-check pass: one real hole (the logout ghost holds wartime), two clears
...
## [2026-09-01] shipped | The fleet mine map: siblings inherit each other's hostile minefields

Operator order (verbatim): "have a mine aware layer between the bots." The fleetshare report gains a `mines` row set — hostile mines only (own-color mines are passable to every same-team sibling by game physics, [[mine-mechanics]]), published within a 60 s share horizon (`MINE_SIGHTING_TTL_MS`) — and the merge lands each sighting through a new `merge_mine_sighting` state mutation that follows the established fleet knowledge law exactly: adds or refreshes, never outranks an equal-or-fresher local belief, stamped `source="world_state"` with `fleet_report` provenance so own-viewport miners keep excluding imports. A merged mine reaches the receiver's composed decision terrain like any remembered mine: walks route around it, teleports expect the displacement, and the corridor clearance shot can drain it — before the receiver ever windows the tile.

Deliberately NO removal half: mines never drift, clearings are visible only in-window, and a cleared mine's phantom re-import self-limits — the clearer stops believing immediately and the sighted copy falls out of every reporter's publication within one share horizon; contact disproofs (0x45 in view, the exact-landing receipt) prune on touch as always. The `fleet_knowledge_merged` diagnostic now counts mines. Codecs carry the full require_* validation with a malformed-row pin; report/merge/mutation each pinned at 100%.

## [2026-09-01] shipped | The fleet gets a lifecycle: no orphans, no killed tanks, and a dashboard that reconnects

Opened by an operator paste: `make fleet` starting clean and then refusing `hud` and `stats` for `yuppler` and `despair` on a loop. The registry is in memory and nothing rehydrated it, so the 404s were a browser tab still holding a dead manager's instance names. `runs/bot/despair/` never existed at all. Then the real orders: "i dont want orhpaned processes", "should we have a make up and a make down command instead of ctrl c", and "when i refreshed the dashboard it took forever to load back".

Four changes, one theme — the manager never knew what it was supervising.

**Draining ([[fleet-lifecycle]]).** An interrupt or `POST /shutdown` asks every live bot to stop and the manager KEEPS SERVING until the last has torn down. It never kills: the same stop sentinel a bounded session ends on, so each tank writes its scorecard and quits to the lobby rather than losing its rank. The wait has no deadline, by ruling. `make down` is a client of that drain, so interrupting the command cannot orphan anything — the manager holds it. `aiohttp.web.run_app` had to go for this: it owns the process and cannot be asked to stop, so the fleet now runs the same `run_until_stopped` loop as the bot service (lifted out of `service_main`, not forked).

**Adoption ([[fleet-lifecycle]]).** Every spawn writes `runs/bot/<instance>/process.json`; every boot re-attaches to the survivors and clears the records of the rest. A record names an IDENTITY — pid AND process creation time, compared exactly — because Windows recycles pids and a manager that adopted on the number alone would grab an unrelated program and then refuse to restart that instance forever. Verified against real processes, including the recycled-pid refusal. Corrupt records RAISE rather than being skipped: records are written atomically, so a bad one is real corruption, and booting past it would silently forget a tank that may still be playing.

**Live reads ([[fleet-live-reads]]).** The slow reconnect was structural, not cosmetic: both summaries re-read and re-decoded the WHOLE events artifact on every cache miss, and yuppler's reached 13.5 MB in six minutes — ~65 MB re-parsed every two seconds across five bots, with the page awaiting each bot in turn. The digest is now a resumable fold (`RunDigestAccumulator`), proven equivalent to the one-pass version at every possible split point, fed by a byte cursor that withholds partial lines and detects a new run by the file's own index. Reads are transactional and a failed fold spoils the run's stream, because advancing past a rejected line would serve numbers with a hole in them.

**Boot identity.** `GET /bots` carries a per-process `boot` id; a page that sees it change reloads instead of aiming 404s at a manager that never heard of its rows. That closes the original symptom.
...
## [2026-09-02] shipped | Engagement doctrines: pluggable per-bot strategy as data, and the swarm muster

Operator order (verbatim: "will we have pluggable strategies that we can swap in per bot or per group of bots at a whim?"). `EngagementDoctrine = skirmish | swarm | duelist | passive` — resolved per bot from `TANKPIT_DOCTRINE` (default `skirmish`, today's exact behavior), carried in `AIConfigDict` with full codecs, and consumed at exactly two chokepoints so a doctrine is DATA selecting between tested gates, never loaded code:

1. **The acquisition choke** (`threat_acquisition._human_rejection_reason`, lifted out of the rejection ladder at the C901 bar): consent and the human rank window stay senior and untouched; the doctrine then rules on WHEN a consented fight opens. `passive` never initiates (`doctrine_passive`); `duelist` takes only an unclaimed duel (`duel_claimed` when a sibling holds it); `swarm` reinforces an engaged sibling instantly but OPENS a fight only when the muster stands — itself plus war-ready siblings ≥ `SWARM_MUSTER_QUORUM` (2), which is the serial-trickle fix. Practice-bot farming is doctrine-blind.
2. **The wartime floor** is now doctrine-scoped to skirmish/swarm — a duelist who cannot claim the duel and a passive bot would otherwise hunt practice bots understocked on a bar meant for a war they will not join.
...
## [2026-09-02] measured | Flag 11 answered: World equipment spawns follow the same law as fuel — no equipment-specific clustering

Operator question (2026-09-01, verbatim: "a massive 5 or 6 equipment containers in one radar. is there any pattern to this. to the spawning across the entire map? have we analyzed it? or is it just luck?"). Analyzed across ALL 106 field05 runs on disk (releases + hot tree): 8,882 distinct own-sensed equipment tiles and 7,889 fuel tiles pulled from the `entity_alignment_sample` belief snapshots (source != world_state, so fleet imports are excluded — the 2026-08-28 miner's discipline).

The instrument is a label-permutation test that makes coverage cancel exactly: equipment and fuel beliefs share the identical sensing footprint (both are window/radar-sensed), so under the null "equipment locations are a random subset of container locations", relabeling which tiles count as equipment among ALL container tiles gives the no-pattern distribution. Result: equipment mean nearest-neighbor distance 1.328 (Chebyshev) vs null 1.322–1.340 (5th–95th pct), **p = 0.36 — dead center of the null**. Whatever spatial law containers follow, equipment follows the SAME one; there are no equipment-specific hotspots to route toward. The per-16x16-block dispersion index of 2.24 (>1) reflects container-rich areas plus coverage accumulation, shared by fuel — which is exactly what the "5–6 in one radar" moments are: ordinary container-density luck, not an exploitable equipment law. Consistent with the 2026-08-28 pickup-atlas finding (7,635 pickups flat across map x) that retired the equipment atlas.

Consequence: NO routing change — the frontier's staleness-seeking uniform treatment of space is what the data supports. Archived with method and numbers at `runs/analysis/equipment-spawn-clustering-20260902.json` beside the earlier atlas archives.

## [2026-09-02] corrected | The dashboard was not slow, it was over a threshold — and the first telling of that page guessed the constant

[[fleet-live-reads]] shipped with the right diagnosis and an unmeasured number: "~65 MB re-parsed every two seconds", which implies seconds of work per poll. A live five-bot fleet finally made it measurable, and the constant is not what that phrasing suggests.

**One whole-file digest of a 23 MB live artifact costs 113 ms.** Timed by calling `build_run_digest` directly on `runs/bot/artax/latest.events.jsonl` (23,130,276 bytes) — that path still backs the CLI, so it measures the pre-change cost faithfully rather than by reconstruction.

So the old design was never slow per read. It failed by CROSSING A LINE: two summaries per bot, whole-file each, awaited bot-by-bot, is ~1.1 s per poll cycle at five bots, against a page that polls every 1 s. Past that point polls overlap and queue on a single-threaded event loop, and the page stops being slow and starts appearing to hang. Five bots is about where the line sits — which is exactly the fleet size at which the operator reported it. The failure is a threshold, not a gradient, which is why it was invisible at one and two bots and total at five.

**After the change, measured on the same live fleet** (85.5 MB across five artifacts, World map): a full page poll — `GET /bots` plus stats and hud for all five — costs 100–120 ms, and a poll landing after the cache TTL (doing a real incremental read and fold) is no slower than one that hits the cache. Per-poll cost no longer depends on how long the run has been going.

The lesson is the one this wiki keeps relearning: a diagnosis can be structurally right and quantitatively wrong, and the wrong quantity hides the actual failure mode. "Re-reading megabytes" pointed at the fix; only the constant showed that the symptom was queue saturation at a specific fleet size.

## [2026-09-02] corrected | Flag 11, second look: no micro-clustering, but real BLOCK-LEVEL equipment bias — the SE quadrant runs ~75% equipment

The operator's "double check ur work" caught it. The earlier same-day entry's no-pattern verdict rested on the nearest-neighbor test, which is UNDER-POWERED at the corpus's tile saturation (mean NN 1.33 — everything is a tile apart, so modest signals drown). A sharper instrument — per-16x16-block equipment FRACTION vs the binomial null, label-permutation — finds real composition bias: observed deviation 413.9 vs null median 327.8, 95th pct 359.3, p < 0.017 (0/60 null draws reached it). Effect size across the 245 well-sampled blocks (n≥40): equipment fraction 0.33–0.76 against a 0.565 global, and the bias is geographically COHERENT — the southeast quadrant (blocks 13-15 × 11-15, centers around (216-248, 184-248)) plus (152, 8-40) run 0.70-0.76, while (120,168) bottoms at 0.33. Better than 2× equipment yield per container encountered between best and worst.

Crucially this measure is DWELL-UNBIASED by construction — a fraction does not grow with visits, which is exactly the trap that killed the pickup atlas. So the corrected doctrine: micro-routing between neighboring tiles buys nothing (the NN result stands), but a **composition prior for the forage frontier when equipment-deficient** — preferring historically equipment-rich blocks among equally-stale candidates — is statistically sound and worth ~2x on the worst-to-best spread. PROPOSED, not built; awaiting operator order. Archive updated in place with the correction section: `runs/analysis/equipment-spawn-clustering-20260902.json`.

---
## [2026-09-01] crack + lift | The join burst measured end to end: the sim was wrong in five places, and 0x74 is a join message

Groundwork for standing the sim up as a multi-client server ([[physics-module-roadmap]]). A single-client sim cannot tell "broadcast to the room" from "send to this client" — both produce identical output — so every unconditional emission in `sim/emissions.py` is an undecided ruling. Seven families had never been decided. All are settled now, recorded in the new [[recipient-policy]].

**The method, and why counts alone were not enough.** Two tests, either sufficient to prove broadcast: the family arrives in a session where the client sent ZERO triggering commands, or it names an actor other than the receiving session's own tank. The first aggregate pass looked decisive for 0x4A (309 received against 59 block commands) and was NOT: [[movable-blocks]] records that dragging emits transient pairs per towed step, so the client's own presses could have produced all 309. The per-session zero-trigger test is what actually settled it — 45 sessions received a 0x4A having sent no block command at all.

**Broadcast:** 0x42 BuildPickup (one sample, `bot-20260826-003928`: own tank 601 received a 0x42 naming tank 709, a drop at (254,9), zero own block commands — thin, and thin for a structural reason, since the bot has no block planner and the whole archive holds 59 block commands) and 0x4A TerrainUpdate (45 sessions). **Per-recipient:** 0x4F, 0x46, 0x4C and TeleportLanded, each with ZERO zero-trigger arrivals against 7,014-12,275 samples.

**0x74 inverted the assumption it was filed under.** 325 of 341 sessions received one having sent no toggle. Characterised: 324 sessions receive exactly ONE, 267 of them at received-message ordinal 48, first arrival median 9.3 s after session start, and 339 of 376 carry the identical payload `(False, True, False, True, True)`. It is a JOIN message delivering the tank's persisted equipment-enabled flags, not an answer to a toggle.

**That question opened the real one.** Measuring where the 0x74 sits meant measuring the whole burst, which is INVARIANT across all 340 archived sessions that carry it:

    0x2Bx2 0x3Dx2 | 0x21 0x3E 0x5A 0x3D 0x2E | 0x21 xN | 0x49 x2 0x74 0x3F

`SimServer.handshake()` diverged FIVE ways, every one invisible at one client: no 0x74 at all; the 0x49 sent once where the real server sends it twice; that 0x49 in the self block where the real pair sits at the TAIL after the identity run; a 0x44 FuelGain invented at join (293 of 340 sessions carry none in their first 90 received messages, the rest 11-36 messages past the sync — the burst's own 0x2E carries fuel, which the sim was also missing, and that is precisely why the real server needs no 0x44 there); and a 0x3D riding every viewport-visible tank in the identity run.

**The fifth is the one with consequences, and it re-derives a known law by a second method.** The 2026-08-04 first-sight probe (3 captures, 113 tanks, every one 0x21-first, 9-46 s to its first position) established the login-roster law; this burst sweep reaches it independently at 340/340 pure-0x21 runs. Not sampling: ~36 tanks on a 256x256 map should put one inside a 16x16 window about 13% of the time, so zero of 340 is the law. The sim had been feeding positions the real server never sends, which means the `(0,0)` phantom — the state production walks on EVERY join, and the entire reason `has_known_position()` and `scripts/state_sentinel_rules.py` exist — was never exercised in sim. `test_handshake_reaches_production_world_state` asserted the enemy at (107,100) after the burst; it now asserts `not has_known_position(enemy)`.

**Verification was a shape diff, not an assertion.** The sim's real `handshake()` output, run-length collapsed against a 37-tank room, matches the archive string exactly — with an in-window tank present in `visible` and still drawing no 0x3D.
...
## [2026-09-02] measured | The fleet coordinates fighting and not foraging, and five bots on one container is that layer working as built

Operator, twice, watching the live five-bot World fleet: "multiple bots go for the same fuel container. i thougt we fixed this but i guess not." Both halves are right, and the second is the interesting one — the container layer WAS built, it is not broken, and it solves a different problem than the one being observed. New page: [[fleet-forage-allocation]].

**The asymmetry, which is invisible until the layers are set side by side.** Enemies share sightings AND coordinate, through focus fire on `engaged_target_id`. Combat timing shares `war_ready` rows AND coordinates, through the swarm muster's quorum of two. Containers share an atlas, tombstones and scan coverage — and coordinate through nothing but an advisory claim. So the fleet already performs positive peer-to-peer coordination with no broker; it just has never been asked to do it for foraging. Five bots converging is the knowledge layer succeeding: it told all five, correctly and promptly, that the container was there, and held no opinion about whose it was.

**The claim is wired end to end and still cannot close it, because of WHEN.** `tick_body` gates the claim on `resource_target_kind != ""` — a bot claims only after it has already selected. Measured across the run: 1,499 pickup dispatches, 273 tiles where two different bots dispatched within 30 s, 1,160 such pairs, **median gap ZERO seconds**. Not a narrow race lost occasionally; siblings commit to the same tile inside the same second, continuously. A message that arrives after the decision cannot arbitrate a collision that happened before it. The claim is not the wrong idea — it is an advisory, eventually-consistent claim doing an authoritative one's job.
...
## [2026-09-02] code + verification | The authoritative container claim lands: exclusive-create mutex closes the median-0s forage race

Operator order off the dispatch-layer discussion (relayed with the fleet-lifecycle session's analysis, both of us converging on the same design): make the container claim REAL. The measured defect ([[fleet-forage-allocation]], independently recomputed this session: 1,499 dispatches, 273 contested tiles, median gap 0 s — my pair count 3,140 under an all-pairs-within-30s definition vs the page's 1,160, same substance) is that the advisory claim publishes after selection, and no message that arrives after the decision can arbitrate a same-tick collision. The filesystem can: exclusive create is atomic.

**The protocol** (`fleetshare/claims.py`): one file per container tile at `runs/bot/_claims/<room>/<x>_<y>.claim` — the leading underscore can never collide with an instance directory because the instance grammar forbids it. Three laws, each pinned by a test: existence is the lock and content is metadata (the create is atomic, the content write is not, so a rival catching the empty file loses one 2 s beat, never a journey); the holder refreshes every full tick and any contender reaps a stamp older than `CLAIM_TTL_MS` = 30 s (sized to ride out the measured 8 s receipt stalls that skip refresh ticks; the reap race is itself re-arbitrated by the retry create); only the owner deletes. The new fs primitive `create_text_exclusive` (O_CREAT|O_EXCL) joins the hook surface with a real-filesystem first-writer-wins pin.

**The integration** (`bot/tick_claims.py`, wired at tick step 6b): reconciliation between decide and execute, not transition tracking — whatever plan the decision wants to persist must own its claim before its command reaches the wire. Denial kills the plan on the spot: `plan_released` reason `claim_lost` (new vocabulary entry), a hold command instead of the doomed dispatch, and the tile stamped into the advisory claimed set to bridge the beat until the winner's own report row lands. Reconciling every tick also self-heals the executor-refusal case where old state persists against a moved claim. The advisory rows STAY — they steer planning away from held tiles so the mutex is rarely even contended; the lock arbitrates the cases steering cannot.

**A scope law learned from the sim.** The first cut raised on "plan with no selected room" as a broken invariant — and the sim seam falsified that within minutes: its direct-entry harness runs whole sessions with plans and no room. The correct law was already on the books: `build_fleet_report` returns None pre-join ("no room, no shareable coordinates"). No room means no fleet channel and nobody to contend with, so arbitration passes through unclaimed. Same contract, two consumers.
...
## [2026-09-02] double-check | The denial stamp died within its own tick — the mutex gains a local denial memory

Operator: "lets double check." One real find in the hours-old claim work: the denial path stamped the losing tile into `ws.fleet_claimed_containers`, and that set is REPLACED WHOLESALE by every merge pass (`merge.py:383`) — so the stamp survived only until step 9 of the same tick. Normally harmless (the winner's own advisory row lands within an exchange), but a winner that crashes right after claiming never publishes an advisory row at all, leaving the loser to re-pick the dead-claimed tile and burn one denied beat per tick for up to the full 30 s claim horizon.

Fix at the root: denials get their own memory. `ws.claim_denied_tiles` (tile -> stamp) is stamped at denial, pruned each arbitration pass, and unioned into the planner's filter at ctx construction through `fresh_denied_claim_tiles` — expiring at exactly `CLAIM_TTL_MS`, because past that horizon the denying claim is itself reapable and re-contending becomes legitimate. Worst case drops from ~15 denied beats to one. Pins: the denial test now asserts the memory and the advisory set's emptiness, a pruning test, a boundary test on the freshness read, and a ctx-filter test proving a denied tile vanishes from `ctx.filtered` exactly like a sibling's advisory row.

Also examined and left standing, recorded so they are not re-found: the stale-own-refresh race (a 30s-stalled holder's refresh replacing a reaper's fresh claim) converges within one tick — the overwritten side gets denied on its next acquire; orphaned `.claim` files from dead sessions are reaped lazily on contention and otherwise accumulate as bytes, a housekeeping note handed to the containerization task (a container start can begin with a clean claims namespace).

## [2026-09-02] code | The fleet goes into a container: one image, two shapes, the runs/ mount as the coordination fabric

Operator order (board task 2dd367b6, claimed this session): containerize the fleet. The single-bot image (ff1fda0c) already had everything but three seams, each of which was a documented limitation rather than a surprise:

1. **The account pool now resolves by env.** `accounts_file_path()` honors `TANKPIT_ACCOUNTS_FILE` before the checkout-relative default — the exact override the Dockerfile header recorded as the missing piece ("multi-account fleet mode is out of scope until that path is env-overridable"). The fleet service mounts `accounts.json` read-only at `/app/accounts.json`; the pool is never baked. Both consumers (account resolution, the fleet page's dropdown via `configured_accounts`) read the one function.
2. **The bind host now resolves by env.** `resolve_fleet_host()` honors `TANKPIT_FLEET_HOST` — the container binds `0.0.0.0` so the published port reaches the process, and the loopback-only security property moves to the docker port mapping (`127.0.0.1:27300:27300`), the container boundary's equivalent of the host default. `fleet_control` (the up/down client) keeps connecting to loopback, which reaches host and container managers alike.
3. **Everything else worked unchanged, verified not assumed:** the spawner forks `sys.executable -c` children (no poetry in the image, none needed); `install_signal_handlers` registers SIGTERM, so `docker stop` IS the drain (`drain_on_interrupt` -> every bot quits to the lobby -> the manager exits after the last one; compose `stop_grace_period: 10m` keeps docker from SIGKILLing a teardown mid-game); the manager stamps each child's `TANKPIT_BOT_SESSION_SECONDS` explicitly, so the image's bounded-session default never leaks into fleet children; the shared `./runs` mount carries knowledge.json, claim files, stop sentinels and spawn records, so a replacement container adopts exactly like a replacement host manager.

Ship shape: one `fleet` service in docker-compose.yml (command `tankpit-fleet`, `shm_size 2gb` for N Chromiums, restart "no"), same image as the single bot. `make image` builds FROM THE NEWEST RELEASE SNAPSHOT (the release tree is the build context, so the tag carries the ladder's sha: `tankpit-fleet:v0.1.0-<sha>`); `make up-docker` resolves the same newest release and refuses to build implicitly (`--no-build`) so a hot-tree image can never masquerade as a release; `make down-docker` is the drain. `make up` (host mode) now prints the fleet page URL on success — the operator asked why it never did.

## [2026-09-02] operator surface | ONE fleet command: `make up` is the containerized fleet, everything else retires

Operator, verbatim: "i dont want like multiple make up commands. like i want one single command we use to start this ish. yk. thats it. one clean system that runs the latest image. and then a make dev if we need. not like a bunch. do we even need make fleet anymore?"

Consolidated to a single pair plus one dev door: **`make up`** resolves the newest release, builds its image if the exact tag (`tankpit-fleet:v<ver>-<sha>`) does not exist yet, and composes the fleet container up with the release's own `runs/` and `accounts.json` mounted, printing the fleet page URL. **`make down`** is the drain (SIGTERM -> every bot to the lobby -> exit, 10 m grace). **`make dev`** runs the manager foreground on the hot tree for development. RETIRED: `make fleet`, `make fleet-dev`, and the same-day `image`/`up-docker`/`down-docker` trio — one system, not two. The `make release` activation hint now says `make down` + `make up`.

The detached HOST lifecycle entry points (`tankpit-fleet-up`/`-down`, `fleet_control.py`) stay in code but leave the operator surface; whether they retire entirely is flagged to their author (fleet-lifecycle session) on the board — they are hours old and the code is the drain client the transition itself needs.
...
## [2026-09-02] triage | Eight Practice-room flags, four root causes — and a nine-minute livelock that cost 20% of the session

Operator watched Artax in PRACTICE (room 7) off release `v0.1.0-fa1c1ae7` and raised eight flags. New page [[flag-triage-20260902]]. Four root causes, two of which are recorded wiki fixes recurring one layer up.

**The livelock (flags 1/4/6).** 21:10:12 → 21:19:17, nine minutes five seconds, 273 viewport shifts, tank frozen at (18,121), zero progress. `plan_block_harvest_leg` latches a fuel lock and frame-shifts; next tick the lock is held-but-not-executable so harvest declines by design; `plan_quad_sweep` takes the tick and its `_anchored_state` calls `clear_resource_target` — deleting the lock — then shifts the window away; free slate, harvest re-latches, repeat 136 times. This **defeats the 2026-08-13 "flag-4 two-shift oscillator" fix**, whose entire mechanism is that the lock survives across ticks: that fix stopped harvest oscillating against *itself* and nothing protects the lock from a peer. The tank was in a two-tile rock pocket, so `walk_or_teleport` could never serve — and it is gated behind `frame_direction(...) is None`, so the movement branch was unreachable throughout. It ended not by resolution but by `FORAGE_COVERAGE_TTL_MS` expiring enough blocks that the frontier hop qualified: **the bot escaped by waiting for its own scan coverage to rot.**

**Clocks standing in for facts (flags 3/4/8).** `FORAGE_COVERAGE_TTL_MS = 180000` exists so "equipment that respawns later is eventually re-discovered", and the neighbouring docstring concedes "container respawn cadence is unmeasured — a working assumption, not a wire-derived law". [[game-economy]] has since FALSIFIED that assumption (605/605 atlas additions were our own exposure; containers gain fuel by deposit, corr(Δv,Δt) = −0.13), and [[enemy-bot-behavior]] records practice bots as non-fuel-seeking and mostly stationary. So in Practice with no humans the container layer is static and swept ground cannot go stale except by our own consumption — the 3-minute clock re-scans a world that cannot have changed, and no coverage clock is room-aware. Measured: 260 frontier hops over 65 blocks, **49% of radars fired within 7 tiles of an earlier radar**, and **143 pickup→teleport→pickup sequences that never left the viewport**. The drain is one step removed from where it looks: `scan_on_landing` is 167 of 179 radar dispatches, so radar burn is a function of teleport count. The quad sweep, the usual suspect, spent 8. Same failure as `flag-triage-20260729` F2, whose patch was a longer clock.

**Absolute thresholds on a rank-variable capacity (flag 1).** `fuel_low_threshold=200` + `hunt_min_fuel=100` are flat, and `make_default_ai_config` says they suit "lieutenant rank" (capacity 1400). Artax is a private (1100). Against a full-health same-rank target the break is unavoidable at EVERY fuel level once rate > 36 — **four hits in ten seconds** — proven live at a literally full tank (fuel=1100, projected 360 < floor 408 → break). The 30 s `kill_cooldown_ms` block TTL then set the shuttle period between the two red pairs.

**Single-slot latches (mine flag).** `mine_pin.py` documents that re-engaging a target never re-presses; false whenever another target intervenes, because `mine_pin_target_id` is one scalar. The A→B→A→B shuttle re-mined two tiles.
...
## [2026-09-02] fix | The lock survives its peers: the walk-territory gap closed, the raw clear guard-restricted, and a progress bound behind it all

The livelock class from [[flag-triage-20260902]] rows 1-2, shipped as one integrated change (design posted to the board before writing). The deeper root found reading at HEAD: the quad sweep's lock wipe was only the AMPLIFIER — the engine was a RESPONSIBILITY GAP in `larder._is_walk_territory`, which ceded any in-viewport container to the walk lane BY GEOMETRY while the walk lane requires an actual route. In the rock pocket both declined the same target forever (the stall's own `hop_declined fuel_larder: too_close=1 stale=34` says it outright — the one fresh candidate WAS the locked target). The F5 water carve-out of 2026-08-01 was this same defect fixed as a special case.

Six changes: (1) the larder's in-viewport cession now asks `is_collection_reachable_in_viewport` on the current surface — the exact predicate the pickup dispatch uses — so the pocket resolves in ONE tick via the larder teleport (pinned in `test_collect_pocket_serving.py`); (2) the equipment hop's `own_ground` deferral asks the same predicate, closing the 2026-07-30 flag-4 gap it had re-opened one gate lower; (3) `plan_quad_sweep` declines while a lock is held and its `clear_resource_target` is DELETED; (4) forage's three coverage decisions preserve a held lock (s11-5: coverage is not pursuit) instead of clearing it silently; (5) the resource-search hop releases via the new enumerated `relocated` reason — same behavior, now visible to churn analysis; (6) `resource_target_held_ticks` + `RESOURCE_LOCK_HOLD_BOUND_TICKS = 8`: the continuation counts dispatch-less holds and releases `progress_stalled` at the bound — the last-resort invariant that converts any unforeseen v3 hold-forever shape into ~16 s plus a diagnostic. No move-failed mark is forged on the stalled release: nothing failed on the wire, and each re-latch cycle is bounded and diagnosed.

Enforcement: new `restricted-symbols` guard rule in `monorepo_guards` (table-driven; AST-based so prose never trips it) — `clear_resource_target` is referencable ONLY in `bot/ai/intent.py`, src+tests+scripts, zero exemptions. The violation class cannot recur silently in any of the 41 packages.
...
## [2026-09-03] fix | The settled-knowledge law: scan staleness becomes a fact question, and the static room stops forgetting

Triage rows 3-5 ([[flag-triage-20260902]]), shipped as one law instead of three patches. The disease: every coverage question ("worth another radar?", "is this ground barren?", "is this block unlooked?") aged out on a wall clock, inventing change no agent could have made — the archive had already falsified every autonomous-change theory ([[game-economy]]: 605/605 dot appearances were our own exposure, refills are deposits with corr(Δv,Δt) = −0.13; [[enemy-bot-behavior]]: practice bots never deliberately collect). Measured cost in the static Practice run: 49% of radars within 7 tiles of an earlier radar, 260 frontier hops over 65 blocks, and — lane-attributed before coding — **139 of the intra-viewport pickup→teleport→pickup sequences were `forage_frontier_hop`**, which collapsed row 5 into row 3 and made row 4 structural (the landing radar was already spend-gated; the gate just read the wrong clock).

The law: a scan stamp is valid while recent OR while it postdates the last moment the world could have changed unobserved — `floor = min(now − ttl, last_foreign_human_seen_ms)`, one threshold expressing both arms (`state/knowledge_floors.py`, split from `scan_coverage` at the ceiling). The watermark is swept from the tank registry by `ws.knowledge_floor_ms` (beliefs mixin): human-named tanks that are neither self nor a fleet sibling advance it monotonically, and it survives the human's 0x58 removal — a departed human still bounds trust in scans from before their presence. Sibling exclusion rides the merge (`fleet_sibling_tank_ids` from the reports' own tank ids, wholesale-replaced like every fleet_* field) because fleet accounts carry human-style names and would otherwise keep every fleet room permanently unsettled. Every coverage predicate went floor-based in place — no defaults, no compat kwargs; `record_scanned_tiles` prunes by the same floor, so a settled room's marks are permanent (bounded by the 256×256 map). The fleet report's publish horizon deliberately stays pure-clock (`ttl_floor_ms`) so shared coverage remains a bounded payload.

Consequences by room: empty Practice → knowledge permanent, frontier churn and redundant radar spend go to zero by construction; human walks in → the floor degrades to exactly the pre-2026-09-03 TTL behavior; human leaves → scans since their departure never age. Row 8 was NOT fixed by this (an earlier board note overclaimed it; corrected in the fix table).
...
Sweep discipline: 35 consumer sites moved to the derived floors in place; `should_proactive_radar` and its stranded helpers turned out to be DEAD production code kept alive only by their own tests — deleted with them. `context.py` crossed the ceiling and split `radar_economics.py` out by role (no re-export shims). Twelve threshold-pinned tests updated with exact rank-2 derivations (171/85/385) rather than loosened; the human-fight scenario needed a hotter phase-3 window to keep exercising the resume-floor branch the smaller reserves had made unreachable — its story is unchanged, its receipts recomputed.

Cleared while gating, on other sessions' committed surfaces (flag-and-fix, stated on the board): `physics.supervisor.shot_refusal` shipped without its machine claim and reverse coverage was red repo-wide — its `law` claim now sits beside the prose in [[capture-differ]]; and the sim's NPC-arm of the refusal branch shipped uncovered — pinned as "refusal receipts belong to the client alone."

Gates: make check green — 6,821 tests, 100.00% statements and branches. Rows 1-6 now await the operator-directed live validation run against the triage baselines.
