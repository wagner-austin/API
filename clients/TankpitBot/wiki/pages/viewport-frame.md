...
[^1]: user (Austin), 2026-05-27 — viewport is a fixed frame, mines under containers. The frame geometry is code truth at `src/tankpit_bot/state/viewport_geometry.py:8-12`: `VISIBLE_VIEWPORT_WIDTH = 16` / `VISIBLE_VIEWPORT_HEIGHT = 16` actionable, widened to the 18x18 observable patch by `RADAR_ENVELOPE_MARGIN` on each side. That it is FIXED rather than tracking the tank is stated at `src/tankpit_bot/state/scan_coverage.py:29`, verbatim: "never moves the window — it moves on teleport landings and on the" scope-shift message.
[^2]: ViewportStateDict and viewport_geometry.py — 18x18 observable, 16x16 actionable, verified via radar fringe reveals
[^3]: `src/tankpit_bot/bot/ai/reachability.py:62` — `is_collection_reachable_in_viewport(world, terrain, start_x, start_y, goal_x, goal_y)`, whose docstring states the completion rule this section relies on: a pickup completes "either by reaching the target tile itself or by reaching a safe cardinally adjacent tile when the target tile cannot be occupied directly (impassable terrain or a hostile mine on the container's tile — both are impassable in the composed terrain view, and both are serviced from an adjacent tile the same way)". Exercised by `tests/bot/ai/test_reachability.py` (8 references), plus the probe-side cases in `tests/action_lab/test_fuel_targeting.py` and `test_fuel_probe_branches.py`. Verified 2026-08-06.
[^4]: user (Austin), 2026-06-21 — "we have viewport shifting off. so the viewport will never move. the only way is to teleport"; contextualised 2026-07-17 as a bot-configuration statement, not a game-rule statement (see [[viewport-shift-protocol]] for the wire mechanism the game supports and the bot doesn't use). Bot-side truth: `src/tankpit_bot/state/scan_coverage.py:29` states the window "never moves" except on teleport landings; the shift path the game does support is `apply_scope_shift` at `src/tankpit_bot/sim/viewport_window.py:92`, which the bot never dispatches.
[^5]: user (Austin), 2026-06-22 — "the corner of the viewport (ir doesnt xenter on pkayer btw)" (typos in the original; the claim is that the viewport is addressed by its corner and does not centre on the player). Encoded in the constructor signature `make_visible_viewport_state(left, top, observed_ms)` at `src/tankpit_bot/state/viewport_geometry.py:15` — the window is built from its top-left origin, never from a tank position, so there is no centring step to get wrong.
[^6]: User (Austin) statement of 2026-06-22, verbatim: "wlak does consume 1 fuel per tile btw". No transcript exists in the repo; the number itself is `WALK_COST_PER_TILE = 1` at `src/tankpit_bot/physics/costs.py:15`, bound as claim `walk-cost` on [[game-economy]] and compared by value on every `make check`. Same statement and same enforcement as [[radar-mechanics]] [^11].
