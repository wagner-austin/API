...
[^1]: user (Austin), 2026-06-16 — full gameplay loop description: combat → refill → radar conservation cycle. The loop is implemented as the mode split: COLLECT entry at `should_enter_collect` (`src/tankpit_bot/bot/ai/mode_gates.py:16`), the refill cascade at `decide_collect_mode` (`src/tankpit_bot/bot/ai/collect_mode.py:199`), and the HUNT readiness bar at `combat_radar_min` (`src/tankpit_bot/bot/ai/mode_gates.py:229`) — the radar-conservation half of the cycle.
