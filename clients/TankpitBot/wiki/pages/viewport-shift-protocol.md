...
[^user]: user (Austin), 2026-07-17 — "auto shift doesnt center on bot every walk. it only shifts when the bot walks to a tile on the edge of the viewport. and then it recenters on bot btw." Reaffirmed: "when you walk to the edge, with auto scroll on, it will center the viewport on the bot." The wire mechanism this describes is `apply_scope_shift` at `src/tankpit_bot/sim/viewport_window.py:92`, with the direction byte enumerated at `:111` and the OFF condition — the configuration the bot actually runs under — at `:88`.
...
[^10]: user (Austin), 2026-07-25, rejecting the walking-traversal suggestion this page briefly carried. Step latency from the run `runs/probe/viewport-20260725-192738.capture_session.json` timeline: single-tile walk commands echoed their 0x47 ~1.7-2.0 s apart.
...
[^shiftlaw]: The anchor-not-stride law is executable in the sim as `apply_scope_shift` at `src/tankpit_bot/sim/viewport_window.py:92`, with the direction byte enumerated at `:111` and the window span `VIEWPORT_SPAN = 16` at `:27`. The bot's standing autoscroll-OFF pin — the reason the human practice above is not the bot's behaviour — is `src/tankpit_bot/browser/autoscroll.py`, whose module docstring at `:1` reads "Wire-verified autoscroll-off enforcement at session start" and quotes the 2026-07-29 ruling at `:9`. Verified present 2026-08-07.
