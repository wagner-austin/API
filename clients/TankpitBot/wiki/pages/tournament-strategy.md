...
Fill AS a recruit before ranking up — abundance drops sharply after the first 15 minutes, and if you rank first you become a target for filled sergeants with dual/homing.[^s2]
...
**Target time:** 8-10 minutes. Under 8 is fast. Over 10 means switching to a kill-heavy or captain-farming approach mid-game to compensate.

## Fill-fighting to lieutenant

At 50+ radars and 60+ of everything else, enable only duals and extras (disable homing), teleport to the nearest bot, sweep on landing with an extra radar, smash to teleport-off, refuel + loot, repeat.[^s3]

Bot shots-to-teleport-off values (7 recruit / 8 private / 9 corporal) plus the last-shade shortcut are on [[enemy-bot-behavior]].

Exit fill-fighting when either (a) you kill a corporal bot as sergeant and rank to lieutenant, or (b) you land on a viewport with a competent human PPH partner and switch to sessioning them.

**Kirby's radar rule (dry maps):** aim for `2 × minutes_remaining` extra radars before mid-game. So at 20 minutes in, 82 ERs is enough to finish. Wet maps (Desert, R&S) need only ~75. If early ER stockpiling isn't feasible, defer it until the second equipment spawn appears and radar count drops to 30-40.
...
**Marathoners:** target teleports every tick, you can only land 1-2 duals before he's gone and you're spending only homings. Stop chasing; let the endgame mob catch him.

**Support-callers:** every attack summons same-color newbies to cock-block, or he beelines to same-color bot clusters for firepower cover. Stop chasing; call your own backup or wait for endgame mobs.

**Self-refillers:** target has enough extras to always find fuel before insufficient. You'll run out of duals before he runs out of shields. Stop chasing.

## Kills vs. PPH balance

Cup requires rank + points. Kills unlock ranks (captain needs 1, major needs 2). PPH decides tie-breaks within a rank. Both matter.[^s5]

Rules of thumb from the guide's community quotes:
...
Homing OFF during PPH — duals only. Duals hit adjacent, cost less inventory to keep full, and don't waste inventory on likely misses. Homing only for kill runs.[^s6]
...
Equipment discipline determines whether refills cannibalize combat time.[^s7]
...
Revelation on homing waste (2014-07-28): *"Don't purposely fire homings over rocks or obstacles unless you're going for a kill. It's a waste, just find a new angle or map."*

## Shield-fighting (endgame)

Reserved for the final 5 minutes if points, not kills, are the cup path.[^s8]
...
Three worked examples from the guide:[^s9]
...
All references are to `docs/sources/sigmas-tankpit-guide-v3.4.pdf`.

[^s1]: §"Some Theory" (kills + PPH + refill triple) and §"Ethics" (top-3 cup structure)
[^s2]: §"Initial equipment fill" — recruit-first fill, 5x5 recruit radar, ferry hunt on water maps, auto-scroll toggling with extras, obstacle-picking economics
[^s3]: §"Fill-fighting to Lieutenant" — dual+ER only, Kirby's 2-ER-per-minute rule, second equipment spawn timing
[^s4]: §"1 – Landing Kills" — Type 1/2/3 taxonomy, 15-20 min sergeant kill window, late-corporal feed-into-sergeant technique, marathoner / support-caller / self-refiller signs
[^s5]: §"Scenario Analysis and Balancing Kills vs. Points" — community quotes from PRIDE, Beaver, Jordann on strategy adaptation
[^s6]: §"2 – How to maximize PPH" — homing-off discipline, bot vs. lieutenant point economics (3 vs 2 shots), water-map bot exception
[^s7]: §"3 – Equipment management" — counter-cyclical filling, fill-as-you-fight, PPH rhythm, shortest-path pickup, endgame fill sizing
[^s8]: §"When to shield fight" — final 5-minute window, clock-hand + 'I' polling, catastrophe threshold
[^s9]: §"Scenario Analysis and Balancing Kills vs. Points" — three worked scenarios
