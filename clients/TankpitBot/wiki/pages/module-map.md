...
source_paths:
  - "src/tankpit_bot"
source_git_blobs:
  "src/tankpit_bot": "547ba3a80ad3e026a2cda669cf3092074db291df"
...
| `bus/` | The three cross-thread session buses and the status contract they carry | `mode_bridge.py`, `status_bus.py`, `frame_bus.py`, `session_status.py`. Below both `bot` and `service`: a standalone session gets inert buses with zero subscribers, the service injects shared ones ([[package-layering]]) |
