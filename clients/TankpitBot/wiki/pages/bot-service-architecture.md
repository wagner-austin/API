...
- **HTTP surface**: `GET /bots` (every instance: pid, alive, returncode, bounds), `POST /bots` `{"instance", "account?", "kills?", "seconds?"}` → 201 (400 malformed, 409 duplicate-live/invalid name), `POST /bots/{instance}/stop` → writes the instance's `runs/bot/<instance>/STOP` sentinel for a graceful boundary exit with full teardown, `DELETE /bots/{instance}` → registry removal, 409 while alive (the fleet never silently kills — stop first).
