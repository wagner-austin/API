...
How the game client renders everything: canvas layer stack, sprite sheets, tile engine, animation system, and dirty-rect invalidation.[^1]

## Canvas Layer Stack

6 named canvas layers, composited via CSS z-index (line 237):[^1]
...
Each layer has a ScaledContext (`qa` class) wrapper that multiplies coordinates by the DPI scale factor.[^1]
...
Two draw modes: `.m` (exact pixel) for scale multiples of 25%, `.s` (ceil-rounded with 1.03× oversize) for non-integer scales. Selected at layer creation time based on scale factor.[^1]

## Tile Engine (og class, lines 148-155)

The game world viewport is an 18×18 grid of `cg` tile objects.[^1]
...
Terrain type 0 = ground, water corners (48-51) are overlays on water (32), obstacles (64+) have their own corner overlays (56-63).[^1]

### Tile Drawing (gd function, line 149-151)

For each dirty tile:[^1]
...
Variable-size frames. Frame offsets stored in parallel arrays:[^1]
- `Cf[]` = frame heights
- `Df[]` = frame widths
- `zf[]` = X draw offsets
- `Af[]` = Y draw offsets
...
Similar variable-frame structure. Dust used for ground movement particles, splash for water.[^1]

### Radar Sprites (He, 68×619)

17-frame sweep animation. Frame offsets in `qf[]` (widths) and `rf[]` (heights).[^1]

## Dirty-Rect System (Hc class, line 36)

Each animated object tracks a bounding rectangle (`Hc` instance):[^1]
```
...
`Jc(a)` — reset after erase.[^1]

Erase sequence: `ra()` → `clearRect(dirty_rect)` → redraw overlapping tanks.[^1]

## Animation Pipeline

Each tick in the action queue:[^1]
...
Multiple animations can run simultaneously in the queue. The tick rate is set to the LAST animation's preferred rate.[^1]

## Map Rendering (Tc class, lines 100-106)

Three canvas buffers:[^1]
...
Map coordinates: `j` and `i` are scroll offsets (0, 64, or 128 in each axis).[^1]

Tank dots: 3×2 pixels per tank, color from team `Y[]` array.
Fuel dots: 1×1 pixels, stored in `aa[]` (x coords) and `la[]` (y coords).[^1]

Own position: Flashing cursor (4 colors cycling at 200ms) at 3×2 pixel size.[^1]


[^1]: JS truth: `tpclient.js` on disk (frontmatter-pinned lines 4/108/142) — `qa` ScaledContext (line 4), tile engine `og`/`cg`/`gd` (lines 145-155), sprite sheets and animation classes (lines 108-127), map `Tc` (lines 100-106), dirty-rect `Hc` (line 36), layer stack (line 237); every claim above carries its (class, line) locator inline, traced 2026-06-19 (frontmatter `verified:` field), re-checkable by grep.
