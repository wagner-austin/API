...
source_git_blobs:
  ...
  "src/tankpit_bot/sniffer": "2a8ea218a2cfc43b6334417f662699f695c9c222"
...
| MUST | **HELLO on human lock (user request 2026-07-29: send "hello" when the bot finishes collecting and has targeted the human player):** the first HUNT decision that locks a NEW human-classified target attaches chat 41 HELLO as its `secondary_command` (`bot/ai/greeting.py::attach_human_greeting`, hooked at the arbitrator's HUNT exit), so the greeting rides the same tick window as the acquisition teleport. Exactly ONE greeting per greeted tank id, latched in `ai_state["greeted_target_id"]` — the server's chat flood mute silently swallows spam for the rest of the session ([[chat-messages]]: 8 echoed then 36 swallowed in sniff-20260729-214411), so the greet is never retried on silence; the inbound 0x4D echo (`chat_received` diagnostic, `is_self_echo`) is the only delivery receipt. Practice bots are never greeted; a decision that already carries a secondary command keeps it (the un-latched greet retries next locked tick). |
