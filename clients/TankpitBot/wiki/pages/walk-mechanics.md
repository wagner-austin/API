...
provenance:
  - "runs/sniff/sniff-20260721-212348.capture_session.json -- gitignored runtime capture artifact (moved from source_paths 2026-09-06, code-paths contract)"
...
0x52 only when static terrain itself severs the corridor.[^simmove]
...
[^3]: same capture as [^2] — three repeated 23-tile manual walks with user narration (input-lock observed live; timing bounds from the wire timestamps). Method and per-episode numbers are recorded in `wiki/log.md` under the 2026-07-21 archive-probe entry, alongside the billing law those walks established.
...
[^5]: user (Austin) 2026-08-04, near-verbatim: "if someone on our team or same tank color scanned the mines previously, they're visible. but if any new mines are planted — even if we're on the same viewport as the planting tank — we cannot see them" is executable in the sim as `_unrevealed_enemy_mine_at` at `src/tankpit_bot/sim/movement.py:188`, which is keyed on the mover's own team rather than on line of sight, so a freshly planted enemy mine stays invisible regardless of viewport overlap. Original wording continues: "even if we're on the same viewport as the planting tank — we cannot see them, unless the mines are for our team of course, or if someone on our team radars." Supplied mid-build while the sim's reveal tracking was being written per-tank; the correction moved it to per-team (`SimWorldDict.revealed_mine_keys_by_team`).
...
[^simmove]: `src/tankpit_bot/sim/movement.py:323` — "The primary plan avoids every blocker the router knows (tanks," …; the router's avoidance set is documented at `:153-155` ("The router avoids other living tanks and VISIBLE enemy mines") and the fallback behaviour at `:283`. Verified present 2026-08-07.
