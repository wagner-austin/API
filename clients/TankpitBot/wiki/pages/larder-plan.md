...
source_git_blobs:
  ...
  "src/tankpit_bot/bot/ai": "920df3b85dccfe84d8f6d4a398062c7b57d7fac6"
...
`collect_mode_outcomes.py:226` calls the same `larder_harvest` on the
under-fire path and takes the hop only when `_hop_escapes_attacker`
(`:131`) confirms it clears the attacker's envelope — so the escape and the
...
``larder_harvest`` (equipment hop first, then fuel), moved AHEAD of
