...
2. **Rejections are not wired to the block/replan machinery.** `mark_move_target_failed` is called from `tick_loop_actions.py:317/:403` (walk failures), `completions.py:232` (completion timeouts), and `game_log_feedback.py:93` (server-side "you can't do that"), but never from any of the executor's nine `emit_ai("rejecting ...")` sites. The `is_move_target_failed` guard at `combat_strategy.py:440` that would route into `block_combat_target_and_replan` is unreachable from executor rejections.[^2]
...
[^2]: grep of `mark_move_target_failed` across `src/`, 2026-07-17: three call sites (`tick_loop_actions.py:317`, `tick_loop_actions.py:403`, `completions.py:232`, `game_log_feedback.py:93`) — none in `executor.py`.
