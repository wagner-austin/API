"""Run the live mine-landing (teleport-onto-mine) probe harness."""
...
def main() -> int:
    """Run the mine-landing-probe entrypoint."""
    from dotenv import load_dotenv

    load_dotenv()
    script_hooks.setup_rich_logging(level="INFO")
    stamp = make_run_stamp()
    configure_probe_runtime_logging("mine-landing", stamp)
    ...
    output_path = (
        _test_hooks.get_env("TANKPIT_MINE_LANDING_OUTPUT")
        or f"runs/probe/mine-landing-{stamp}.json"
    )
    headless = _parse_bool_env(_test_hooks.get_env("TANKPIT_HEADLESS"))
    max_attempts = int(_test_hooks.get_env("TANKPIT_MINE_LANDING_MAX_ATTEMPTS") or "3")
    max_extras = int(_test_hooks.get_env("TANKPIT_MINE_LANDING_MAX_EXTRAS") or "6")
    initial_sync_timeout_ms = int(
        _test_hooks.get_env("TANKPIT_MINE_LANDING_INITIAL_SYNC_TIMEOUT_MS") or "10000"
    )
    ...
    log.info(format_mine_landing_probe_summary(session))
