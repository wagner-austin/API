"""Run the live fuel-dot verification probe harness."""

from __future__ import annotations

from platform_core.logging import get_logger

from scripts import _test_hooks as script_hooks
from tankpit_bot import _test_hooks
from tankpit_bot.action_lab import (
    FuelDotProbeSessionDict,
    format_fuel_dot_probe_summary,
    run_fuel_dot_probe,
)
from tankpit_bot.runtime_logging import configure_probe_runtime_logging

log = get_logger(__name__)


def _parse_bool_env(value: str | None) -> bool:
    """Parse a boolean-like environment variable.

    Args:
        value: Raw environment value or None.

    Returns:
        True for truthy values ("true", "1", "yes"), False otherwise.
    """
    return value is not None and value.lower() in ("true", "1", "yes")


def _parse_optional_int_arg(argv: list[str], flag: str) -> int | None:
    """Parse an optional integer CLI flag value.

    Args:
        argv: Process argument vector.
        flag: Flag name to look up.

    Returns:
        Parsed integer value, or None when the flag is absent.

    Raises:
        ValueError: If the flag is present without a value.
    """
    if flag not in argv:
        return None
    index = argv.index(flag)
    if index + 1 >= len(argv):
        raise ValueError(f"{flag} requires an integer value")
    return int(argv[index + 1])


def _format_saved_path(output_path: str) -> str:
    """Return the saved-path log line payload.

    Args:
        output_path: Session JSON output path.

    Returns:
        Log line text.
    """
    return f"Saved to: {output_path}"


def main() -> int:
    """Run the fuel-dot verification probe entrypoint.

    Returns:
        Process exit code (0 on success).
    """
    from dotenv import load_dotenv

    load_dotenv()
    script_hooks.setup_rich_logging(level="INFO")
    configure_probe_runtime_logging("fuel_dot")

    if _test_hooks.sync_playwright is None:
        _test_hooks.sync_playwright = _test_hooks.get_sync_playwright()

    argv = _test_hooks.get_argv()
    max_dots = _parse_optional_int_arg(argv, "--max-dots")
    initial_sync_timeout_ms = _parse_optional_int_arg(argv, "--initial-sync-timeout-ms")

    target_url = _test_hooks.get_env("TANKPIT_URL") or "https://tankpit.com/play"
    output_path = _test_hooks.get_env("TANKPIT_FUEL_DOT_PROBE_OUTPUT") or "fuel_dot_probe.json"
    headless = _parse_bool_env(_test_hooks.get_env("TANKPIT_HEADLESS"))
    prefer_account = _parse_bool_env(_test_hooks.get_env("TANKPIT_PREFER_ACCOUNT"))
    env_initial_sync_timeout_ms = int(
        _test_hooks.get_env("TANKPIT_FUEL_DOT_INITIAL_SYNC_TIMEOUT_MS") or "10000"
    )
    env_max_dots = _test_hooks.get_env("TANKPIT_FUEL_DOT_MAX_DOTS")
    acquisition_timeout_ms = int(
        _test_hooks.get_env("TANKPIT_FUEL_DOT_ACQUISITION_TIMEOUT_MS") or "3000"
    )
    teleport_timeout_ms = int(
        _test_hooks.get_env("TANKPIT_FUEL_DOT_TELEPORT_TIMEOUT_MS") or "10000"
    )
    radar_timeout_ms = int(_test_hooks.get_env("TANKPIT_FUEL_DOT_RADAR_TIMEOUT_MS") or "5000")
    settle_delay_ms = int(_test_hooks.get_env("TANKPIT_FUEL_DOT_SETTLE_MS") or "500")

    session: FuelDotProbeSessionDict = run_fuel_dot_probe(
        target_url,
        output_path,
        headless=headless,
        prefer_account=prefer_account,
        max_dots=max_dots if max_dots is not None else int(env_max_dots or "6"),
        initial_sync_timeout_ms=(
            initial_sync_timeout_ms
            if initial_sync_timeout_ms is not None
            else env_initial_sync_timeout_ms
        ),
        acquisition_timeout_ms=acquisition_timeout_ms,
        teleport_timeout_ms=teleport_timeout_ms,
        radar_timeout_ms=radar_timeout_ms,
        settle_delay_ms=settle_delay_ms,
    )
    log.info(format_fuel_dot_probe_summary(session))
    log.info(_format_saved_path(output_path))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())


__all__ = [
    "main",
    "run_fuel_dot_probe",
]
