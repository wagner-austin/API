"""Run the live combat accuracy probe harness."""

from __future__ import annotations

from platform_core.logging import get_logger

from scripts import _test_hooks as script_hooks
from tankpit_bot import _test_hooks
from tankpit_bot.action_lab.combat_probe import (
    format_combat_probe_summary,
    run_combat_probe,
)
from tankpit_bot.action_lab.combat_probe_types import CombatProbeSessionDict
from tankpit_bot.runtime_artifacts import make_run_stamp
from tankpit_bot.runtime_logging import configure_probe_runtime_logging

log = get_logger(__name__)


def _parse_bool_env(value: str | None) -> bool:
    """Parse a boolean-like environment variable."""
    return value is not None and value.lower() in ("true", "1", "yes")


def _parse_optional_int_arg(argv: list[str], flag: str) -> int | None:
    """Parse an optional integer CLI flag value."""
    if flag not in argv:
        return None
    index = argv.index(flag)
    if index + 1 >= len(argv):
        raise ValueError(f"{flag} requires an integer value")
    return int(argv[index + 1])


def main() -> int:
    """Run the combat probe entrypoint."""
    from dotenv import load_dotenv

    load_dotenv()
    script_hooks.setup_rich_logging(level="INFO")
    stamp = make_run_stamp()
    configure_probe_runtime_logging("combat", stamp)

    if _test_hooks.sync_playwright is None:
        _test_hooks.sync_playwright = _test_hooks.get_sync_playwright()

    argv = _test_hooks.get_argv()
    max_engagements = _parse_optional_int_arg(argv, "--max-engagements")
    max_shots = _parse_optional_int_arg(argv, "--max-shots")

    target_url = _test_hooks.get_env("TANKPIT_URL") or "https://tankpit.com/play"
    output_path = _test_hooks.get_env("TANKPIT_COMBAT_PROBE_OUTPUT") or (
        f"runs/probe/combat-{stamp}.json"
    )
    headless = _parse_bool_env(_test_hooks.get_env("TANKPIT_HEADLESS"))
    prefer_account = _parse_bool_env(
        _test_hooks.get_env("TANKPIT_PREFER_ACCOUNT"),
    )

    session: CombatProbeSessionDict = run_combat_probe(
        target_url,
        output_path,
        headless=headless,
        prefer_account=prefer_account,
        max_engagements=max_engagements or 3,
        max_shots_per_engagement=max_shots or 20,
    )
    log.info(format_combat_probe_summary(session))
    log.info("Saved to: %s", output_path)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())


__all__ = [
    "main",
    "run_combat_probe",
]
