"""Dump raw 0x5A viewport entities from a captured session.

Usage: poetry run python -m scripts.analyze_viewport_entities [session.json]
"""
...
def main() -> None:
    """Dump raw viewport entities from a capture session."""
    ...
    try:
        result = analyze_viewport_entities(session)
    except ValueError as exc:
        sys.stdout.write(f"{exc}\n")
        raise SystemExit(1) from exc

    sys.stdout.write(format_viewport_entity_dump(result))
    sys.stdout.write("\n")
