...
_STANDOFF_PROFILES: dict[str, CombatProfile] = {
    **_NAVAL_PROFILES,
    ...
    # An armed structure with the reach of a fortress gun: the enemy
    # command centre that held the first wiring on from frame 0.
    "enemy_hall": profile("enemy_hall", 400.0),
}

#: Mobility for the standoff picture: everything the tests field moves,
#: except the hall -- speed zero is what tells a building from a unit,
#: the same read :func:`mobile_threats` runs.
_STANDOFF_CATALOGUE = {
    "c_tank": unit_stats("c_tank"),
    "c_missile": unit_stats("c_missile"),
    "builder": unit_stats("builder", armed=False),
    "enemy_arty": unit_stats("enemy_arty"),
    "enemy_probe": unit_stats("enemy_probe"),
    "warship": unit_stats("warship"),
    "heli": unit_stats("heli"),
...
def test_nothing_seen_reads_as_not_outranged() -> None:
    ...
    assert not ground_outranged(("c_tank",), (), _STANDOFF_PROFILES, _STANDOFF_CATALOGUE)
...
def test_a_longer_land_gun_in_sight_reads_as_outranged() -> None:
    ...
    assert ground_outranged(mix, (_arty(1),), _STANDOFF_PROFILES, _STANDOFF_CATALOGUE)
...
def test_a_tie_is_a_fair_fight_not_a_standoff() -> None:
    ...
    assert not ground_outranged(("c_tank",), picture, _STANDOFF_PROFILES, _STANDOFF_CATALOGUE)
...
def test_fliers_and_ships_belong_to_the_other_clauses() -> None:
    ...
    assert not ground_outranged(("c_tank",), (_heli(1),), _STANDOFF_PROFILES, _STANDOFF_CATALOGUE)
    assert not ground_outranged(("c_tank",), (_ship(1),), _STANDOFF_PROFILES, _STANDOFF_CATALOGUE)
...
def test_an_armed_structure_never_arms_the_clause() -> None:
    ...
    enemy's own command centre is armed, outranges everything, and stands
...
def test_a_mix_with_no_land_gun_reads_as_not_outranged() -> None:
    ...
    assert not ground_outranged(("builder",), (_arty(1),), _STANDOFF_PROFILES, _STANDOFF_CATALOGUE)
