...
from rw_bot.policy.doom import COLUMNS, DoomError, DoomLatch, DoomWatch, decode_doom_model
...
def test_a_model_decodes_with_its_scalars_and_features() -> None:
    model = decode_doom_model(
        _model_lines('{"name": "army_mean", "mean": 5.0, "std": 2.0, "coef": 1.5}')
    )
...
def test_an_empty_file_a_bad_window_and_a_bad_std_stop_loudly() -> None:
    with pytest.raises(DoomError) as caught:
        decode_doom_model([])
    assert caught.value.code == "RW-DOOM-001"
    with pytest.raises(DoomError) as caught:
        decode_doom_model(['{"window": 0, "threshold": 0.7, "intercept": 0.0}'])
    assert caught.value.code == "RW-DOOM-001"
    with pytest.raises(DoomError) as caught:
        decode_doom_model([_HEAD, '{"name": "army_mean", "mean": 0.0, "std": 0.0, "coef": 1.0}'])
    assert caught.value.code == "RW-DOOM-001"
    with pytest.raises(DoomError) as caught:
        decode_doom_model([_HEAD])
    assert caught.value.code == "RW-DOOM-001"
