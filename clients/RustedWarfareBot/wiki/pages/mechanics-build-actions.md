...
## The rally point and the tier unlock are the same shape, and price is what separates them

An action that concerns no type is not necessarily an upgrade. The land factory's set-rally action and its tier-two unlock both reach the wire as `produces:""`, `placed:false`, `makes_something:false`, `available:true` — the first live tech probe selected the first no-type action it found and dispatched four rally points at a booked 2,000 credits each.[^15] The two classes differ in exactly one accessor: the action's cost, `c()`, abstract on the action base class and answered by every action — setRally returns 0, the tier upgrade returns its tier's price (`ar.b.c(2)`, 2,000 for the land factory), and a produce action returns its unit's price.[^16]

So the option stream carries the engine's own charge as `price`, and the tech channel selects the no-type action that costs something, claiming the wire's figure rather than a catalogue guess.[^17] The recaptured world sample shows the discriminator live: the command centre's rally at price 0 beside its builder action at 500.[^18]

**The per-action index is not a selector, and the interned key is the dispatch handle.** The engine exposes an int accessor per action (`t()`), and every action on a unit answers the same figure — the recapture shows a command centre's rally, builder and scout actions all reporting `1` — so a dispatch that resolved "the action at index N" always returned the first action on the list, which is the rally point. The probe that had selected the *right* option by price still fired the wrong action for exactly this reason. The only stable per-action identity is the interned key (`c_1`, `u_builder`, …), the same object the engine's own executor resolves commands by, so the option stream publishes `key` and the ability order fires the key back.[^19]
...
[^1]: `runs/decompiled/com/corrodinggames/rts/game/units/a/s.java:245` — `y()` returns the placed type and defaults to null on the base; `g()` is abstract. `a/l.java` is the production family: it overrides `g()` to return true, and builds its key as `"u_" + as2.v()` in its constructor.
...
[^9]: `agent/src/rwbot/agent/BuildOptions.java:163` — `ownedOptions`, whose filter is the union of the two tests, with the two failed single-test versions recorded in its comment.
[^10]: `agent/src/rwbot/agent/Orders.java:362` — `produce`, and `BuildOptions.gatesOf` / `BuildOptions.Gates.closed` for the readings and the refusal message.
...
[^15]: `runs/techprobe3.log` / `runs/techprobe4.out` — four `ability: selector 1 via action 'c_1'` dispatches, `tech:landFactory got 4 spent 8000`, and no heavy type ever produced; `runs/decompiled/com/corrodinggames/rts/game/units/a/o.java:19` names `c_1` as `gui.actions.setRally`.
[^16]: `runs/decompiled/com/corrodinggames/rts/game/units/a/s.java:140` — `public abstract int c()`; `a/o.java:33` returns 0 for the rally, `d/n.java:36` returns `ar.b.c(2)` for the land factory's tier upgrade, `a/l.java:47` returns the produced unit's price via `this.B().a()`.
[^17]: `agent/src/rwbot/agent/BuildOptions.java` (`Option.price`, read via `EngineNames.ACTION_PRICE`), `src/rw_bot/wire/state.py` (`BuildOption.price`), `src/rw_bot/policy/spending.py` (`unlock_tech`).
[^18]: `wiki/sources/m6-wire/world-sample.ndjson` — regenerated 2026-07-31 through `make wire-capture`; unit 213's option records carry `produces:"", key:"c_1", price:0` (rally) and `produces:"builder", key:"u_builder", price:500` adjacent.
[^19]: `runs/techprobe5.log` — the price-selected unlock still dispatching `c_1`, four times; the pre-key capture's option records all carrying `action:1` for unit 213; `agent/src/rwbot/agent/BuildOptions.java` (`actionByKey`, `keyNameOf`), `src/rw_bot/wire/command.py` (`AbilityOrder.key`).
