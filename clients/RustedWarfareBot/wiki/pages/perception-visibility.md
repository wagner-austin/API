...
[^5]: `agent/src/rwbot/agent/StateStream.java:31` — the record shapes; hit points come from `Orders.healthOf`, which reads the pair the engine itself divides for a health fraction (`cu / cv`).
[^6]: `agent/src/rwbot/agent/Perception.java:21` — `visibleEntities` calls the pinned `am.d(n)` per entity; the name lives in `EngineNames.VISIBLE_TO` and `EngineNames.verifyBindings` checks that method against the jar so a moved name fails at `make check`.
