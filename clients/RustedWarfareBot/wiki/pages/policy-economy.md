...
[^10]: `.decompiled/com/corrodinggames/rts/gameFramework/j/ah.java:38` — the room settings object, whose `b()` dumps `startingCredits`, `fogMode`, `aiDifficulty`, `startingUnits`, `incomeMultiplier` and `randomSeed`; `incomeMultiplier` is field `h`, initialised to `1.0f`.
