...
source_paths:
  ...
  - "runs/sweeps/"
source_git_blobs:
  "doctrines/flame-nocover.doctrine": "a5dd7f1da45a76199966375cefc0362ad95796aa"
provenance:
...
| Hard (duel_lake, modern policy) | `evolve1-g4m2` | **188/192 on the wide certification** (hardwide192, 2026-09-06, two preemption waves converged by resubmission); with the prior day's panels, **259/264 on current code, 98.1 percent** (Wilson 95 percent CI ~0.8-4.4 percent tail) | The certification stalemate did not reproduce on current code. Two knobs asked and refused at panel grade 2026-09-05: close 3 turned 3 wins into WIPES, and nukes 1 -- a 12/12 screen sweep -- fell at verdict grade. **The tail is now SIZED and its anatomy read** (hardwide192's 4 non-wins: 2 defeated, 2 stalemated at the 10,000-sample cap): every one is a LONG match (8,280-10,000 samples vs the winning median 2,414) where the rival compounds past 120k worth and the deaths come from mixed AIR-SEA attrition -- gunShips, lightGunships, helicopters, amphibiousJets, battleShips -- not the pure navy grind the 1/72 read suggested. A match the champion fails to close early drifts into an attrition regime its ground army cannot win. **The tail is a COIN, not a curse** (hardtail48, 2026-09-06): all four non-win seeds replayed 12x under the unchanged champion read **48/48 won** at medians indistinguishable from the fleet's -- nothing is anchored to a seed, and the mechanism traces to the process-varying AI evaluation schedule ([[policy-determinism]], 2026-09-06). Rung total on current code: **307/312, 98.4 percent**, five non-wins, none reproducible. Moving a ~1.6 percent coin needs wide certification per lever arm (faster close, or the banked navy/AA machinery for the attrition regime) -- or the shared-root instrument fix, the per-call-site draw routing, which would shrink this tail and the Impossible noise floor together. |
| Hard, Linux+pinned+ff10 (duel_lake) | `aa-counter-guard` | v8 detection, held to v7 across 120 paired seeds (ab24 + ab48 + ab48b, 2026-09-01): v7 102/120 won (85%) vs v8 90/120 (75%); 0 armyless, 0 losses, 0 stalls in all 240 matches | The pace question is UNRESOLVED, LEANING AGAINST v8 -- not certified noise (power audit 2026-09-08, [[power-audit-mde-ledger]]): discordant pairs 25:13, two-sided p = 0.073, and the sign test carries only 0.44 power against its own observed split at n=120, so "noise" was never certifiable here. The per-wave dilution (7:2, 9:4, 9:7) stays suggestive, not probative. Trace-pinned mechanism of divergence: arms are bit-identical until the first refusal-recovery moment (the watch's ~2-sample report replacing the 45-sample clock), after which matches are different matches. Labeled hypothesis, untested: the early asymmetry tracked the contended-boot seam, which wave three (node-local clones, 96/96 zero failures) removed. v8 keeps the rung; detection is defect-critical. Windows-path figures do not compare. |
...
| Impossible (duel_lake) | `evolve3-g3m10` (standing base, adopted 2026-09-06); wins still 0 | The first SURVIVAL adoption: the imp-survival CEM graduate (c_tank x2 + heavyTank x2 + c_artillery -- double heavy armor, hover eliminated) holds **+510.6 paired survival at t=4.20 and +340.0 at t=2.56 on two disjoint 48-pair blocks** (e3stand96/e3standrep, stand-longer 63/96, medians 3,694/3,447 vs 3,086/3,230). Prior margin-era champion `flame-nocover` stands as the base the search perturbs. The win path (turtle->bank->nuke) builds Phase B on this fortress; evolve4 climbs from it. |
| Impossible margin era (superseded record) | `flame-nocover` | 0/~120 post-certification, survival floor ~3300; 0/48 vs `flame-close6` (imp48c6) AND 0/48 vs `close0-flame4` (imp48c0f4), both paired | No adoptable gain, and composition is now measured OFF the table: close6 washed (+0.051) and the full VH champion close0-flame4 washed too (-0.082, sd 0.784) -- one was a closing behavior that never acted, the other shapes the army from the first engagement, and neither moved. What wins at VH is orthogonal to what Impossible punishes -- confirmed a THIRD time by the machine-learned champion itself: g4m2imp48 read 0/48 both sides, paired margin +0.210 (sd 0.607), a small survival lean and no road to a win. The next gain requires a new capability class -- and by 2026-09-05 every candidate in the credit vocabulary is measured closed: denial, rebuild, strike, brace, and the opening scaffold (see the frontier notes below). Stage 3 -- policy above the vocabulary -- is the road ([[impossible-economy-problem]]). Champion held by default of evidence. |
...
- Updated 2026-09-05: TWO more closures land -- the brace (impbrace48b: the razing head armed 48/48 inside its horizon, margin -0.029, the fourth mechanism-proven endgame closure) and the opening scaffold (impopen96 screen flat everywhere at screen power -- own-spread MDEs 253-583 samples across the seven arms, so only e1's escalated question is closed at the SEI; tankfirst's +263 at t=2.29 sits at its own MDE boundary and was never panel-tested, recorded as an open cheap question in [[power-audit-mde-ledger]]; e1's rival-compounding dent REFUTED at verdict grade by impe1v48: margin +0.053 at sd 0.799, flips 25:23). The match is now closed from the first build order to the death phase; the credit-allocation vocabulary has no unmeasured layer left. Stage 3 of the staged road -- per-sample policy, or a posture built against the scripted AI's group ladder ([[engine-ai-zones]]) -- is the standing frontier ([[impossible-economy-problem]]).
- Updated 2026-09-05 (later): the SIXTH closure -- stage 3's cheap end. The hunt (a standing party pressing forming groups, the first verb aimed at the wave LADDER, built same-day on the m32 live capture) measured flat at screen tier with the mechanism verifiably firing, in BOTH its arms at once: hand-tuned at three party sizes and recalled by the razing head (the first learned per-sample decision driving a live tactical response). imphunt60b: margins -0.156 to +0.093, survival flat AT SCREEN POWER (own-spread survival MDEs 264-494 samples across the four arms, two above the 340-sample SEI -- the hands are refused, not proven null; [[power-audit-mde-ledger]]), zero wins, hunts-median 18-28 where the first screen's starved-draft defect had read zero. Stage 3's remaining road is only the expensive architecture lane ([[impossible-step-three-design]]); that spend is the operator's call.
- Updated 2026-09-06: the INSTRUMENT is re-priced -- detpair24 (12 identical-doctrine pairs on shared seeds, difficulty 3) reads paired-survival sd 1,205 samples with the verdict label flipping on 7 of 12 seeds, and four SAME-NODE pairs forking, which the rng ledger traces to draw-count variance in the engine and math streams ([[policy-determinism]], the 2026-09-06 section). Minimum detectable paired effect: ~850 samples at n=8, ~700 at n=12, ~350 at n=48. Every n<=12 read at this rung is priced accordingly; the two survival adoptions stand on their 48-pair blocks (+510.6 at t=4.20, +340.0 at t=2.56).
- Updated 2026-09-07: the instrument work PAID -- the routing weaves (sway + effect-jitter draws off the sim stream) and the ledger's own-draw fix nearly HALVED the floor: detpair24b reads **sd 661.8** on 12 fresh identical pairs (F=3.31 vs detpair24, significant at n=12). Minimum detectable paired effect now ~468 at n=8, ~382 at n=12, **~191 at n=48** -- a 48-pair panel resolves what previously needed ~190 pairs. All future screens size against the new floor; historical panels keep their own eras' pricing.
- Updated 2026-09-06 (later): the SEVENTH closure -- the income ladder. income_ladder on the standing base funded the tier-three conversion in 48/48 matches (the base funds it in none) and read paired survival -201.0 (sd 878.5, t=-1.59), lean negative, no detectable gain (impincome96, 48 pairs sized against the floor). A T3 conversion's ~500-second payback does not fit inside a ~790-second median match -- the bank's trade on the income side. All three of the bank verdict's reopeners are now closed or nonexistent; the win path funnels to the per-sample policy lane ([[impossible-step-three-design]]).

## Closed questions (measured, with the log entry that closed them)

**Very Hard:**
- The mid-game read family, both responses, on the 96 certified bar seeds (log 2026-09-13, [[very-hard-race]]): the press (release everything at sample 2,000 when behind) read -4 on 48 pairs; the turtle (hold home from sample 1,500 when behind) reads 57 to the champion's 69 with twelve seeds lost and none gained, every one a seed the champion wins from behind; and the riposte counter-punch, alone or behind any threshold of the hold (70, 85, 100), reads 62 on every arm -- the same seven seeds gained, fourteen lost, the riposte alone and the 70 threshold matching seed for seed. Attacking less after a mid-game read is closed in both directions, the counter-punch is a net -7, and no knob in the vocabulary wins the behind-at-1,500 cohort more often than the champion's own continued pressure.
- The interception, on the same seeds (log 2026-09-14, [[very-hard-race]]): off reads 59 to the champion's 69 (15 gained, 25 lost), so the reserve turning on raiders is worth ten wins; bounded to three units reads 68 (11 gained, 12 lost), the largest flip count of any arm on the bar for a net of minus one. Motivated by a kill-ledger read that put the losses' worse exchange in the 1,000-1,500 window down to something other than numbers; that read was a confound (the rival's size at 1,000 understates what the waves fought, and binned by its size at 1,500 the gap closes -- log 2026-09-14, the retraction), so the rejection stands on its own and the exchange deficit is the rival's income roll at the fight.
