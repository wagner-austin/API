...
| Very Hard (duel_lake) | `flame-close6` | 35/96 paired fresh seeds (36%) vs `flame-nocover`'s 20/96 on the same seeds | ADOPTED 2026-09-02 (laws six and nine: +8 then +7 against the +4 bar, combined flips 29:14, p=0.031; one knob from `flame-nocover`: `close 3` -> `close 6`) -- and CHALLENGED the same day by its own neighborhood: vhsearch3's graduate `close0-flame4` (flame 2->4, latch deleted) beat it **28-14 of 48** (+14 vs the +4 bar, flips 16:2, p=0.001, c0f4vh) -- a 58% win rate where no VH configuration ever read above 44%. Law nine (`c0f4rep`, seeds 30001-34893) in flight; the champion changes only if it replicates. |
