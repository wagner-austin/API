...
|---|---|---|---|
| Hard, Linux+pinned+ff10 (duel_lake) | `aa-counter-guard` | v8 detection: 15 won + 9 survived-with-army of 24, 0 armyless, 0 stalls (hard24-detect + ab24-v8, replicated exactly) | Standing reference for the pinned regime, MEASURED against v7 by paired A/B (ab24, 2026-09-01): v7 20+4 vs v8 15+9 on the same seeds, 9 flips splitting 7:2 -- within chance (two-sided p~=0.18), mean army higher under v8, no losses either arm. Trace-pinned mechanism: arms are bit-identical until the first refusal-recovery moment, where the watch's ~2-sample report replaces the 45-sample clock and the matches become different matches. Detection stays: it is defect-critical, and the win-count delta is not distinguishable from butterfly variance at 24 pairs. Windows-path figures do not compare. |
