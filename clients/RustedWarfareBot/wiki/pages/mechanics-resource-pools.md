...
source_paths:
  ...
  - "src/rw_bot/policy/siting.py:514"
...
[^10]: `agent/src/rwbot/agent/StateStream.java:79` — `poolRecord` and the `pools` count on the frame record; `agent/src/rwbot/agent/MapTiles.java` holds the scan, the cache and the fog filter.
