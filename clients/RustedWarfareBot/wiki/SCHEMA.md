...
Enforcement runs in two tiers, split by what a machine can honestly assert (amended 2026-09-07, the day the first CI run on a fresh clone met the old single-tier rule). The **repo tier** — everything above except artifact existence — is a property of the checkout alone, so the test suite enforces it everywhere, fresh clones and CI runners included. The **artifact tier** additionally existence-checks paths under the artifact roots (`runs/`, `.game/`, `.decompiled/` — the authoritative tuple is `ARTIFACT_ROOTS` in `scripts/wiki_check.py`), so a typo'd artifact path still fails loudly — but only where the artifact store lives. The checker decides that from the tree (amended 2026-09-20, the day `make check` began running on CI runners and fleet nodes): the store is held when every artifact root is a directory, and only then, because `runs/` alone is what the test suite leaves behind anywhere and `.game/` alone is the Linux game tree CI links in, which lacks the launcher scripts the pages cite. Where the store is held, `make sources` (and therefore `make check`) existence-checks every artifact citation and says how many; anywhere else it reports the tier as not applicable, names the absent roots and counts the citations it did not check, so a pass never reads as more than it examined. A fresh clone genuinely does not hold the measurement record, and a gate that only one machine can pass gates nothing anywhere else.
